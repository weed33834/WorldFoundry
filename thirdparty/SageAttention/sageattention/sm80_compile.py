...
@torch.library.custom_op("sageattention::qk_int8_sv_f16_accum_f16_fuse_v_mean_attn_window_size", mutates_args=("output",), device_types="cuda")
def qk_int8_sv_f16_accum_f16_fuse_v_mean_attn_window_size(
    query: torch.Tensor, 
    key: torch.Tensor, 
    value: torch.Tensor, 
    output: torch.Tensor, 
    ...
    Custom CUDA kernel for SageAttention with INT8 quantization for Q and K, FP16 PV with FP16 accumulation.
    """
    return _qattn_sm80.qk_int8_sv_f16_accum_f16_fuse_v_mean_attn_window_size(
        query, key, value, output,window_size_left,window_size_right, query_scale, key_scale, value_mean,
        tensor_layout, is_causal, qk_quant_gran, sm_scale, return_lse
    )

@torch.library.register_fake("sageattention::qk_int8_sv_f16_accum_f16_fuse_v_mean_attn_window_size")
def qk_int8_sv_f16_accum_f16_fuse_v_mean_attn_fake_window_size_impl(
    query: torch.Tensor, 
    key: torch.Tensor, 
    value: torch.Tensor, 
    output: torch.Tensor, 
