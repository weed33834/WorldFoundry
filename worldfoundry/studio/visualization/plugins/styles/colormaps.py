...
def colorize_depth(
    ...
    if normalize:
        ...
        disp = (disp - min_disp) / max(float(max_disp - min_disp), 1e-8)
...
def colorize_depth_affine(depth: np.ndarray, mask: np.ndarray = None, cmap: str = "Spectral") -> np.ndarray:
    ...
    depth = (depth - min_depth) / max(float(max_depth - min_depth), 1e-8)
...
def colorize_disparity(
    ...
    if normalize:
        ...
        disparity = (disparity - min_disp) / max(float(max_disp - min_disp), 1e-8)
...
def colorize_error_map(
    ...
    colorized_error_map = cmap(((error_map - vmin) / max(float(vmax - vmin), 1e-8)).clip(0, 1))[..., :3]
    if mask is not None:
