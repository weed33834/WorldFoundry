...
from worldfoundry.core.attention import apply_rotary_embedding, varlen_scaled_dot_product_attention
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.config.rope:
            ...
            qh = apply_rotary_embedding(qh, cos_h, sin_h)
            kh = apply_rotary_embedding(kh, cos_h, sin_h)
            qw = apply_rotary_embedding(qw, cos_w, sin_w)
            kw = apply_rotary_embedding(kw, cos_w, sin_w)
            query_states = torch.cat([qh, qw], dim=-1)
            key_states = torch.cat([kh, kw], dim=-1)

        attn_output = varlen_scaled_dot_product_attention(
