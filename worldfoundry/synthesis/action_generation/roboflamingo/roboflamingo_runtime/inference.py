...
class RoboFlamingoRuntimeConfig:
    ...
    def to_plan_dict(self) -> dict[str, Any]:
        return {
            ...
            "runtime_package": "worldfoundry.synthesis.action_generation.roboflamingo.roboflamingo_runtime",
...
def select_roboflamingo_runtime_config(
    *,
    ...
    architecture = RoboFlamingoArchitectureConfig(
        llm_name=str(options.get("llm_name") or "mpt_dolly_3b"),
        vision_encoder_path=str(options.get("vision_encoder_path") or "ViT-L-14"),
        vision_encoder_pretrained=str(options.get("vision_encoder_pretrained") or ""),
        cross_attn_every_n_layers=int(options.get("cross_attn_every_n_layers") or 1),
        window_size=int(options.get("window_size") or options.get("eval_hist_size") or 12),
        fusion_mode=str(options.get("fusion_mode") or "post"),
        use_gripper=_bool_option(options, "use_gripper", True),
        use_state=_bool_option(options, "use_state", False),
        decoder_type=str(options.get("decoder_type") or "lstm"),
        head_type=str(options.get("head_type") or "lstm"),
        precision=str(options.get("precision") or "fp32"),
    )
...
class RoboFlamingoRuntime:
    ...
    def _load_model_bundle(self) -> tuple[Any, Any, Any]:
        ...
        from robot_flamingo.models.factory import create_model_and_transforms
        ...
        openflamingo_state = torch.load(self.config.openflamingo_checkpoint_path, map_location="cpu")
        model.load_state_dict(openflamingo_state, strict=False)
        policy_state = torch.load(self.config.policy_checkpoint_path, map_location="cpu")
    ...
    def predict_action(
        ...
        *,
        ...
        prediction = _coerce_action_trace(getattr(output, "logits", output))
