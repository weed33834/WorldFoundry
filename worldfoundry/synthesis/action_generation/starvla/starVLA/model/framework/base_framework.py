...
_IMPORTED_FRAMEWORKS: set[str] = set()


def _import_framework_module(framework_id: str) -> None:
    if framework_id in _IMPORTED_FRAMEWORKS:
        return

    _SKIP = {"__init__", "base_framework", "share_tools"}
    framework_dir = Path(__file__).resolve().parent
    target = framework_id.lower()
        ...
        if is_pkg:
            ...
            for _, sub_name, _ in pkgutil.iter_modules([str(sub_dir)]):
                if sub_name.startswith("_"):
                    continue
                if sub_name.lower() == target:
                    importlib.import_module(f"starVLA.model.framework.{module_name}.{sub_name}")
                    _IMPORTED_FRAMEWORKS.add(framework_id)
                    return
        elif module_name.lower() == target:
            importlib.import_module(f"starVLA.model.framework.{module_name}")
            _IMPORTED_FRAMEWORKS.add(framework_id)
            return

    # Last resort: import the small package initializers so registry aliases added
    # there remain visible, but do not import every framework implementation.
    for _, module_name, is_pkg in pkgutil.iter_modules([str(framework_dir)]):
        if is_pkg and module_name not in _SKIP:
            importlib.import_module(f"starVLA.model.framework.{module_name}")
    _IMPORTED_FRAMEWORKS.add(framework_id)
...
def build_framework(cfg): # The single entry point for building different model frameworks
    ...
    _import_framework_module(framework_id)
