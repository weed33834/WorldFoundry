...
# Official InternVLA-A1 Transformers 4.57.1 Qwen3-VL inference implementation.
#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
...
try:
    ...
    def check_model_inputs(func):
        """Inference-only compatibility wrapper for Transformers 5.x.

        Transformers 4.57 used this decorator to resolve cache/return defaults
        and optionally record intermediate outputs.  The retained InternVLA
        inference path never requests output recording, but it does rely on the
        cache and tuple-conversion behavior.
        """
...
class Qwen3VLTextRotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: Qwen3VLTextConfig, device=None):
        ...
        if not isinstance(rope_config, dict):
            rope_config = {}
        self.rope_type = rope_config.get("rope_type", "default")
        ...
        self.mrope_section = rope_config.get("mrope_section", [24, 20, 20])

    @staticmethod
    def _compute_default_rope_parameters(config: Qwen3VLTextConfig, device=None):
        """Transformers-4.57-compatible default RoPE initialization."""
        ...
        if not isinstance(rope_config, dict):
            rope_config = {}
        base = rope_config.get("rope_theta", getattr(config, "rope_theta", 5_000_000.0))
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if past_key_values is not None:
            if use_cache:
                ...
                # Transformers 5 stores cache tensors on per-layer objects and
                # removed ``DynamicCache.__getitem__``.  Keep the 4.57 path for
                # older releases while reading the non-mutating prefix cache
                # through the new API when present.
                if hasattr(past_key_values, "layers"):
                    cache_layer = past_key_values.layers[self.layer_idx]
                    cached_key_states = cache_layer.keys
                    cached_value_states = cache_layer.values
                else:
                    cached_key_states = past_key_values[self.layer_idx][0]
                    cached_value_states = past_key_values[self.layer_idx][1]
...
class Qwen3VLForConditionalGeneration(Qwen3VLPreTrainedModel, GenerationMixin):
    _checkpoint_conversion_mapping = {}
    # Transformers 5 represents tied weights as target->source mappings;
    # Transformers 4.57 (the release environment) used a target-name list.
    _tied_weights_keys = (
