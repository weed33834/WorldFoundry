...
from olmo.torch_util import get_world_size, get_rank
# from olmo_core.utils import ensure_multiple_of
from olmo.util import ensure_multiple_of


class CPLoadBalancerType(StrEnum):
    """
    An enumeration of the different :class:`CPLoadBalancer` implementations.
    """

    zig_zag = "zig_zag"
    """
    ➡️ :class:`RingAttentionZigZagLoadBalancer` (for ring attention)
    """

    llama3 = "llama3"
    """
    ➡️ :class:`RingAttentionLlama3LoadBalancer` (for ring attention)
    """

    ulysses = "ulysses"
    """
    ➡️ :class:`UlyssesLoadBalancer` (for Ulysses attention)
    """

    def build(self, cp_mesh: DeviceMesh) -> "CPLoadBalancer":
        """
...
RingAttentionLoadBalancerType = CPLoadBalancerType


class CPLoadBalancer(metaclass=ABCMeta):
    """
    A class that handles the logic of sharding inputs on the sequence dimension
    for context parallelism (CP). This is used by different CP strategies including
    ring attention and Ulysses attention.
    """
...
RingAttentionLoadBalancer = CPLoadBalancer


class RingAttentionZigZagLoadBalancer(CPLoadBalancer):
    """
    Implements the zig-zag load-balancing strategy for ring attention.
    """
...
class UlyssesLoadBalancer(CPLoadBalancer):
    """
    Simple sequential load-balancing strategy for Ulysses attention.
    Each rank gets a contiguous chunk of the sequence: rank i gets chunk i.
    This is simpler than zig-zag and matches the communication pattern of Ulysses attention.
    """
    ...
    def batch_shard(
        ...
        *,
        ...
        for x, seq_dim, pad_value in zip(
            ...
            # Simple sequential chunking: rank i gets chunk i
            x_chunks = x.chunk(self.cp_world_size, dim=seq_dim)
            local_value = x_chunks[self.cp_rank].contiguous()
            out.append(local_value)
    ...
    def batch_shard_by_document(
        ...
        *,
        ...
        raise NotImplementedError(
            f"{self.__class__.__name__} does not support intra-document masking yet. "
            "Use 'batch_shard()' for simple sequence sharding."
        )
...
class RingAttentionLlama3LoadBalancer(CPLoadBalancer):
    """
