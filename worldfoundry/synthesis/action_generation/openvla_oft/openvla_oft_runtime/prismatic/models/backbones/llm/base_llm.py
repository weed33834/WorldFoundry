...
def _resolve_local_hf_mirror(repo_id: str) -> str:
    """Prefer a WorldFoundry local HF mirror when it exists; otherwise keep native HF loading."""
    ...
    if hfd_root:
        candidates.append(Path(hfd_root).expanduser() / safe_name)
    if ckpt_root:
        root = Path(ckpt_root).expanduser()
        candidates.extend([root / "hfd" / safe_name, root / safe_name])
...
class HFCausalLLMBackbone(LLMBackbone, ABC):
    def __init__(
        ...
        resolved_hf_hub_path = _resolve_local_hf_mirror(hf_hub_path)
        ...
        if not self.inference_mode:
            overwatch.info(
                f"Loading [bold]{llm_family}[/] LLM from [underline]`{resolved_hf_hub_path}`[/]",
                ctx_level=1,
            )
            self.llm = llm_cls.from_pretrained(
                resolved_hf_hub_path,
                token=hf_token,
        ...
        # [Contract] `inference_mode` means we're loading from a pretrained checkpoint; no need to load base weights!
            ...
            overwatch.info(
                f"Building empty [bold]{llm_family}[/] LLM from [underline]`{resolved_hf_hub_path}`[/]",
                ctx_level=1,
            )
            llm_config = AutoConfig.from_pretrained(resolved_hf_hub_path, token=hf_token)
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            resolved_hf_hub_path, model_max_length=self.llm_max_length, token=hf_token, padding_side="right"
        )
