...
WorldFoundry modifications keep the fixed, parameter-free MEM inference path
and dispatch spatial attention to optional FA2 or an in-tree PyTorch SDPA
fallback. The state-dict layout is unchanged.
...
class SpaceTimeBlock(nn.Module):
    ...
    def _qkv(self, h: torch.Tensor):
        ...
        a = self.attn
        batch_tokens, seq_len, _ = h.shape
        if a.q_bias is not None:
            ...
            qkv = a.qkv(h)
        q, k, v = (
            qkv.reshape(batch_tokens, seq_len, 3, a.num_heads, a.head_dim)
            .permute(2, 0, 3, 1, 4)
            .unbind(0)
        )
    ...
    def _time_softmax_on_v(self, q, k, v, b: int, kf: int) -> torch.Tensor:
        ...
        def reshape_to_time(t):
            return (
                t.view(b, kf, heads, n, d)
                .permute(0, 3, 2, 1, 4)
                .reshape(b * n, heads, kf, d)
            )
    ...
    def _space_attn(self, q, k, v, cu_slens=None) -> torch.Tensor:
        ...
        from .modeling_hunyuan_vl_mot import _spatial_attention_forward

        a = self.attn
        bk, _, n, _ = q.shape
        out = _spatial_attention_forward(
            q,
            k,
            v,
            scaling=a.scale,
    ...
    def forward(self, x: torch.Tensor, cu_slens=None, num_frames: int = 1) -> torch.Tensor:
        ...
        if kf > self.max_num_frames:
            raise ValueError(
                f"num_frames={kf} exceeds configured maximum {self.max_num_frames}"
            )
...
def apply_video_encoder_patch(
    ...
    Args:
        ...
        max_num_frames: validated upper bound on K (history length).
