"""LingBot-VLA v2 observation preprocessing and action postprocessing."""
...
from .transforms import Normalizer, prepare_images, prepare_state, prepare_language, prepare_action, expert_visual_transform
from .ee_pose import *
...
class FeatureTransform:
    def __init__(
        ...
        # Keep declared order: list-valued observations rely on stable camera
        # positions and Python set iteration varies with the hash seed.
    ...
    def get_normalizer(self, norm_stats_path, do_nomalize, data_config):
        ...
        action_state_norm_type = {
            key: value
            for item in data_config.norm_type
            for key, value in (dict(item) if isinstance(item, dict) else ast.literal_eval(item)).items()
        }
