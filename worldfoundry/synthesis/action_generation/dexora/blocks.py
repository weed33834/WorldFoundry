...
from worldfoundry.core.attention import QKNormRopeSelfAttention, scaled_dot_product_attention
from worldfoundry.core.nn import Mlp


class RmsNorm(nn.Module):
    """Checkpoint-compatible RMSNorm used by the released RDT blocks."""
...
class TimestepEmbedder(nn.Module):
    ...
    def __init__(self, hidden_size, frequency_embedding_size=256, dtype=None):
        super().__init__()
        if dtype is None:
            raise ValueError("TimestepEmbedder dtype must be resolved by the inference runtime")
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, c: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        x = scaled_dot_product_attention(query=q, key=k, value=v, dropout_p=0.0, attn_mask=mask)
...
class RDTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, **block_kwargs):
        ...
        self.attn = QKNormRopeSelfAttention(
            dim=hidden_size, num_heads=num_heads, qkv_bias=True, qk_norm=True, norm_layer=RmsNorm, **block_kwargs
        )
        self.cross_attn = CrossAttention(
            hidden_size, num_heads=num_heads, qkv_bias=True, qk_norm=True, norm_layer=RmsNorm, **block_kwargs
        )
...
def get_multimodal_cond_pos_embed(embed_dim, mm_cond_lens: OrderedDict, embed_modality=True):
    ...
    if embed_modality:
        ...
        modality_sincos_embed = get_1d_sincos_pos_embed_from_grid(embed_dim // 2, np.arange(num_modalities))
    ...
    for idx, (modality, cond_len) in enumerate(mm_cond_lens.items()):
        if modality == "image" and (isinstance(cond_len, tuple) or isinstance(cond_len, list)):
            ...
            cond_sincos_embed = get_1d_sincos_pos_embed_from_grid(
                pos_embed_dim, np.arange(cond_len if cond_len > 0 else 1)
            )
