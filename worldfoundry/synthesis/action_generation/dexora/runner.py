...
class RDTRunner(nn.Module):
    def __init__(
        ...
        *,
        ...
        if dtype is None:
            raise ValueError("RDTRunner dtype must be resolved by the inference runtime")
    ...
    def conditional_sample(self, lang_cond, lang_attn_mask, img_cond, state_traj, action_mask, ctrl_freqs):
        ...
        noisy_action = torch.randn(
            size=(state_traj.shape[0], self.pred_horizon, self.action_dim), dtype=dtype, device=device
        )
        action_mask = action_mask.expand(-1, self.pred_horizon, -1)

        # Set step values
        self.noise_scheduler_sample.set_timesteps(self.num_inference_timesteps, device=device)
        ...
        for t in self.noise_scheduler_sample.timesteps:
            ...
            model_output = self.model(
                state_action_traj, ctrl_freqs, t.unsqueeze(-1).to(device), lang_cond, img_cond, lang_mask=lang_attn_mask
            )
