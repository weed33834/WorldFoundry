...
def load_video(
    ...
    **kwargs,
    ...
    Args:
        video (`VideoInput`):
            The video to convert to the numpy array format. String inputs must be local paths;
            runtime network acquisition is deliberately disabled.
    ...
    if parsed.scheme or parsed.netloc:
        raise ValueError(
            "MolmoAct2 video inputs must be local files; URL and URI loading is disabled."
        )
    if os.path.isfile(video):
        file_obj = video
    else:
        raise FileNotFoundError(f"MolmoAct2 video path does not exist: {video}")
...
class MolmoAct2VideoProcessor(BaseVideoProcessor):
    ...
    def fetch_videos(
        ...
        """
        Convert a single local path or a list of local paths into `np.array` objects.

        If a single path is passed, the return value is a single object. A list produces
        a list of decoded objects. Network URLs are rejected by :func:`load_video`.
        """
        if (
