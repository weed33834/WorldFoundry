...
from worldfoundry.base_models.diffusion_model.video.ltx2.ltx_core.components.diffusion_steps import EulerCfgPpDiffusionStep, Res2sDiffusionStep
from worldfoundry.base_models.diffusion_model.video.ltx2.ltx_core.components.protocols import DiffusionStepProtocol
from worldfoundry.base_models.diffusion_model.video.ltx2.ltx_core.model.transformer import X0Model
from worldfoundry.base_models.diffusion_model.video.ltx2.ltx_core.utils import to_denoised, to_velocity
from worldfoundry.synthesis.visual_generation.ltx2.ltx_pipelines.utils.helpers import post_process_latent, timesteps_from_mask
from worldfoundry.synthesis.visual_generation.ltx2.ltx_pipelines.utils.res2s import get_res2s_coefficients
from worldfoundry.synthesis.visual_generation.ltx2.ltx_pipelines.utils.types import Denoiser, LatentState
...
def euler_denoising_loop(
    ...
    denoiser:
        ...
        and must return a :class:`~worldfoundry.synthesis.visual_generation.ltx2.ltx_pipelines.utils.types.DenoisedLatentResult`.
    ### Returns
    tuple[LatentState | None, LatentState | None]
...
def euler_cfg_pp_denoising_loop(
    ...
    Requires a guided denoiser whose :class:`~worldfoundry.synthesis.visual_generation.ltx2.ltx_pipelines.utils.types.DenoisedLatentResult`
    ...
    video_state:
        Current video :class:`~worldfoundry.base_models.diffusion_model.video.ltx2.ltx_core.types.LatentState`, or ``None``.
    audio_state:
        Current audio :class:`~worldfoundry.base_models.diffusion_model.video.ltx2.ltx_core.types.LatentState`, or ``None``.
    stepper:
        :class:`~worldfoundry.base_models.diffusion_model.video.ltx2.ltx_core.components.diffusion_steps.EulerCfgPpDiffusionStep`
        instance carrying ``eta`` and ``s_noise`` parameters.
    transformer:
        The diffusion model passed to the denoiser at each step.
    denoiser:
        Callable implementing :class:`~worldfoundry.synthesis.visual_generation.ltx2.ltx_pipelines.utils.types.Denoiser`.
    noise_seed:
