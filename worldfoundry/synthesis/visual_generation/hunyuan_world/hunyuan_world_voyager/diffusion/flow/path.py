...
class VPCPlan(ICPlan):
    ...
    def __init__(self, sigma_min=0.1, sigma_max=20.0, reverse=False):
        ...
        self.log_mean_coeff = (
            lambda t: -0.25 * ((1 - t) ** 2) * (self.sigma_max -
                                                self.sigma_min) - 0.5 * (1 - t) * self.sigma_min
        )
        self.d_log_mean_coeff = lambda t: 0.5 * \
            (1 - t) * (self.sigma_max - self.sigma_min) + 0.5 * self.sigma_min
        self.reverse = reverse
        if self.reverse:
    ...
    def compute_sigma_t(self, t):
        ...
        d_sigma_t = th.exp(p_sigma_t) * \
            (2 * self.d_log_mean_coeff(t)) / (-2 * sigma_t)
