...
def load_vision_encoder(
    ...
    if vision_encoder_type == "siglip":
        subfolder = "image_encoder" if os.path.isdir(os.path.join(vision_encoder_path, "image_encoder")) else None
        kwargs = {"subfolder": subfolder} if subfolder else {}
        vision_encoder = SiglipVisionModel.from_pretrained(vision_encoder_path, **kwargs)
    else:
...
def load_image_processor(processor_type, processor_path=None, logger=None):
    ...
    if processor_type == "siglip":
        subfolder = "feature_extractor" if os.path.isdir(os.path.join(processor_path, "feature_extractor")) else None
        kwargs = {"subfolder": subfolder} if subfolder else {}
        processor = SiglipImageProcessor.from_pretrained(processor_path, **kwargs)
    else:
