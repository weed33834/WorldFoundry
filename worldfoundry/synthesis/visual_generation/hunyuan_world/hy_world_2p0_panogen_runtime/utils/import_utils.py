...
def is_torch_deterministic() -> bool:
    """
    Check whether pytorch uses deterministic algorithms by looking if
    torch.set_deterministic_debug_mode() is set to 1 or 2"
    """
    if is_torch_available():
        import torch
...
def is_torchvision_v2_available() -> bool:
    ...
    # NOTE: We require torchvision>=0.15 as v2 transforms are available from this version:
...
def is_bitsandbytes_multi_backend_avail() -> bool:
...
def is_in_notebook() -> bool:
    try:
        ...
        if ("DATABRICKS_RUNTIME_VERSION" in os.environ and
                os.environ["DATABRICKS_RUNTIME_VERSION"] < "11.0"):
            # Databricks Runtime 11.0+ uses IPython kernel by default,
            # so it should be compatible with Jupyter notebook
...
def is_sudachi_projection_available() -> bool:
    ...
    # NOTE: We require sudachipy>=0.6.8 to use projection option in sudachi_kwargs
    # for the constructor of BertJapaneseTokenizer.
    # - `projection` option is not supported in sudachipy<0.6.8,
    #   see https://github.com/WorksApplications/sudachi.rs/issues/230
...
def create_import_structure_from_path(module_path):
    ...
    """
    directory = _normalize_import_module_path(module_path)
    import_structure, adjacent_modules = _scan_import_directory(directory)
    adjacent_modules = _filter_adjacent_modules(adjacent_modules)
    module_requirements = _build_module_requirements(directory, adjacent_modules)
    return {**module_requirements, **import_structure}


def _normalize_import_module_path(module_path):
    if os.path.isfile(module_path):
        return os.path.dirname(module_path)
    return module_path


def _scan_import_directory(directory):
    import_structure = {}
    adjacent_modules = []
    for entry in os.listdir(directory):
        entry_path = os.path.join(directory, entry)
        if entry != "__pycache__" and os.path.isdir(entry_path):
            import_structure[entry] = create_import_structure_from_path(entry_path)
        elif not os.path.isdir(entry_path):
            adjacent_modules.append(entry)
    return import_structure, adjacent_modules


def _filter_adjacent_modules(adjacent_modules):
    ...
    if "__init__.py" in adjacent_modules:
        adjacent_modules = [module for module in adjacent_modules if module != "__init__.py"]

    # Modular files should not be imported
    has_modular = any("modular_" in module for module in adjacent_modules)
    has_modeling = any("modeling_" in module for module in adjacent_modules)
    if has_modular and has_modeling:
        adjacent_modules = [module for module in adjacent_modules if "modular_" not in module]

    return adjacent_modules


def _build_module_requirements(directory, adjacent_modules):
    module_requirements = {}
    for module_name in adjacent_modules:
        ...
        base_requirements = _get_base_requirements(module_name)
        exported_objects = _parse_requires_exports(file_content, module_name, base_requirements, module_requirements)
        _export_all_objects(file_content, module_name, base_requirements, exported_objects, module_requirements)

    return module_requirements


def _get_base_requirements(module_name):
...
def _parse_requires_exports(file_content, module_name, base_requirements, module_requirements):
    exported_objects = set()
    if "@requires" not in file_content:
    ...
    for index, line in enumerate(lines):
        ...
        if "@requires" in previous_line:
            backends = _extract_requires_backends(previous_line, lines, previous_index, index)
            ...
            if not line.startswith("class") and not line.startswith("def"):
                ...
                _update_module_requirements(module_requirements, backends, module_name, object_name)
...
def _extract_requires_backends(previous_line, lines, previous_index, current_index):
    ...
    if "backends" in previous_line:
        ...
        return tuple(sorted([b.strip("'\",") for b in backends_string.split(", ") if b]))

    # Backends are defined in the lines following export.
    if previous_index + 1 < len(lines) and "backends" in lines[previous_index + 1]:
        backends = []
        for backend_line in lines[previous_index:current_index]:
...
def _update_module_requirements(module_requirements, backends, module_name, object_name=None):
...
def _export_all_objects(file_content, module_name, base_requirements, exported_objects, module_requirements):
    ...
    for _all_object in fetch__all__(file_content):
        if _all_object not in exported_objects:
            ...
            _update_module_requirements(module_requirements, backends, module_name, _all_object)
