...
"""
Distributed model checkpoint (DCP) directory structure and storage backends.
...
from worldfoundry.core.io.s3_filesystem import S3StorageReader, S3StorageWriter
...
def save_checkpoint_in_background(
    ...
    This function runs in a dedicated process to avoid blocking model checkpoint writes.
    ...
    Args:
        ...
        job_config: Runtime job metadata.
    ...
    checkpoint_handler = DistributedCheckpointer(checkpoint_config, job_config, disable_async=True)
...
class DistributedCheckpointer(AbstractCheckpointer):
    ...
    def __init__(
        ...
        super().__init__(config_checkpoint, config_job)
    ...
    def load(
        ...
        if checkpoint_path is not None and not checkpoint_path.endswith(".pth"):
            ...
            log.info("No checkpoint path configured.")
        torch.cuda.empty_cache()
    ...
    def get_previous_checkpoint_results(self, wait_for: int = 0) -> None:
        """Get the results of previously submitted model checkpoint saves."""
        if self.async_mode == AsyncMode.ASYNC_WITH_PINNED_MEM:
            try:
                start_time = time.monotonic()
    ...
    def save(
        ...
        """Save model weights to a DCP checkpoint."""
        if self.async_mode == AsyncMode.ASYNC_WITH_PINNED_MEM:
