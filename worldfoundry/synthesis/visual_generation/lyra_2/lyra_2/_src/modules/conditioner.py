...
class TextAttrEmptyStringDrop(AbstractEmbModel):
    ...
    def random_dropout_input(
        ...
        if self.empty_prompt_data is None:
            self.empty_prompt_data = easy_io.load(str(self._resolve_empty_prompt_path()))
    ...
    def _resolve_empty_prompt_path(self) -> Path:
        candidates: list[Path] = []
        if self.empty_prompt_path:
            candidates.append(Path(self.empty_prompt_path).expanduser())
        env_path = os.environ.get("WORLDFOUNDRY_LYRA2_EMPTY_PROMPT_PATH")
        if env_path:
            candidates.append(Path(env_path).expanduser())
        candidates.append(Path(__file__).resolve().parents[2] / "checkpoints" / "empty_string_umt5.pt")
        for candidate in candidates:
            if candidate.exists():
                return candidate
        searched = ", ".join(str(path) for path in candidates)
        raise FileNotFoundError(
            "Text dropout requires an empty prompt embedding checkpoint. "
            "Pass empty_prompt_path or set WORLDFOUNDRY_LYRA2_EMPTY_PROMPT_PATH. "
            f"Searched: {searched}"
        )
