...
def load_model_dcp(model, ckpt_path):
...
def load_model_torch_cpu(model, ckpt_path):
    """Load the state_dict on host memory before distributed synchronization."""
    if distributed.is_rank0():
        ckpt = easy_io.load(
            ckpt_path,
            map_location="cpu",
...
class UMT5EncoderModel:
    def __init__(
        ...
        checkpoint_path="./checkpoints/text_encoder/encoder.pth",
        tokenizer_path="google/umt5-xxl",
        enable_fsdp_shard: bool = False,
        load_on_cpu: bool = False,
    ):
        assert not enable_fsdp_shard, "FSDP is not supported for UMT5"
        checkpoint_path = os.environ.get("LYRA2_TEXT_ENCODER_CKPT", checkpoint_path)
        tokenizer_path = os.environ.get("LYRA2_UMT5_TOKENIZER", tokenizer_path)
        ...
        if checkpoint_path.endswith(".dcp"):
            model = load_model_dcp(model, checkpoint_path)
        else:
            assert checkpoint_path.endswith(".pth"), "only .pth or .dcp are supported"
            if load_on_cpu:
                model = load_model_torch_cpu(model, checkpoint_path)
            else:
...
def get_umt5_embedding_offloaded(
    ...
    """
    Load UMT5 encoder on CPU only, move to CUDA just for the call, then back to CPU and clear CUDA cache.
    """
    global _t5_offloaded
    if _t5_offloaded is None:
        _t5_offloaded = UMT5EncoderModel(device="cpu", load_on_cpu=True)
    _t5_offloaded.model.to(device)
    _t5_offloaded.device = device
    emb = _t5_offloaded(prompts, device=device)
