...
from inference_planning.evaluator import PlanEvaluator
...
try:
    import submitit
except Exception:  # pragma: no cover - optional batch launcher dependency
    submitit = None
...
def launch_plan_jobs(
    ...
    if submitit is None:
        raise RuntimeError("DINO-WM launch_plan_jobs requires submitit, but submitit is not installed.")
...
class PlanWorkspace:
    def __init__(
        ...
        if self.wandb_run is None or (
            wandb is not None and isinstance(self.wandb_run, wandb.sdk.lib.disabled.RunDisabled)
        ):
        ...
        from inference_planning.mpc import MPCPlanner
...
def planning_main(cfg_dict):
    ...
    if cfg_dict["wandb_logging"]:
        if wandb is None:
            raise RuntimeError("DINO-WM wandb_logging=True requires wandb, but wandb is not installed.")
