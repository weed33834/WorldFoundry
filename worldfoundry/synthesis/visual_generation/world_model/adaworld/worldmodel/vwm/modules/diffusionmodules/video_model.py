...
from vwm.util import default, repeat_img_seq
...
class VideoResBlock(ResBlock):
    def __init__(
        ...
        super(VideoResBlock, self).__init__(
            channels,
            emb_channels,
            dropout,
...
class VideoUNet(nn.Module):
    ...
    def forward(
        ...
        if context.shape[0] != x.shape[0]:
            assert context.shape[0] == x.shape[0] // self.num_frames
            context = repeat_img_seq(context, self.num_frames)

        if self.num_classes is not None:
            if y.shape[0] != x.shape[0]:
                assert y.shape[0] == x.shape[0] // self.num_frames
                y = repeat_img_seq(y, x.shape[0] // y.shape[0])
