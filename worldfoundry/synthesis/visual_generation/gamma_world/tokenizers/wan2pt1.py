...
from worldfoundry.core.time import BenchmarkTimes
from worldfoundry.synthesis.visual_generation.gamma_world.tokenizers.interface import VideoTokenizerInterface
from worldfoundry.synthesis.visual_generation.gamma_world.tokenizers.wan2pt1_2d_plugins import plugin_mount
...
def _video_vae(
    ...
    **kwargs,
    ...
    if pretrained_path is None:
        raise ValueError("vae_pth is required for Gamma inference")
    else:
        if get_rank() == 0:
            from worldfoundry.core.io.easy_io import get_checkpoint_path
            ...
            if load_mean_std:
                if mean_std_path is None:
                    raise ValueError("mean_std_path is required when load_mean_std=True")
                ...
                img_mean, img_std = easy_io.load(img_mean_std, map_location=device)
                video_mean, video_std = easy_io.load(video_mean_std, map_location=device)
...
class WanVAE:
    ...
    def _broadcast_split_for_model_parallelsim(self, state: torch.Tensor) -> torch.Tensor:
        # All ranks in the context-parallel group encode different inputs before the later broadcast.
