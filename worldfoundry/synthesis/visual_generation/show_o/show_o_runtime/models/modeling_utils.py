...
def _load_state_dict_compat(model_file: str | os.PathLike[str], **kwargs):
    try:
        return load_state_dict(model_file, **kwargs)
    except TypeError as exc:
        if "variant" not in kwargs or "variant" not in str(exc):
            raise
        kwargs = dict(kwargs)
...
def _load_model_dict_into_meta_compat(
    model,
    state_dict,
    *,
    device,
    ...
    try:
        result = load_model_dict_into_meta(
            model,
            state_dict,
            device=device,
            dtype=dtype,
        ...
        if from_flax:
            ...
            if low_cpu_mem_usage:
                ...
                if device_map is None and not is_sharded:
                    ...
                    unexpected_keys = _load_model_dict_into_meta_compat(
                        model,
                        state_dict,
                        device=param_device,
                        dtype=torch_dtype,
