...
def setup_pipeline(args: argparse.Namespace, text_encoder=None):
    ...
    set_random_seed(seed=args.seed, by_rank=True)
    # Initialize cuDNN.
...
def process_single_generation(
    ...
    if video is not None:
        ...
        if pipe.config.state_t == 16:
            ...
            fps = 16
        save_image_or_video_tensor(video, output_path, fps=fps)
...
def generate_video(args: argparse.Namespace, pipe: Video2WorldPipeline) -> None:
    ...
    if args.batch_input_json is not None:
        ...
        f = open(args.batch_input_json, "r")
        lines = f.readlines()
        batch_inputs = [json.loads(x) for x in lines]

        for idx, item in enumerate(batch_inputs):
            log.info(f"Processing batch item {idx + 1}/{len(batch_inputs)}")
            input_video = item.get("video_path", "")
            videoname = input_video.split("/")[-1]
            # prompt = item.get("prompt", "")
            prompt = item.get("dense_lang", "")
            batch_output_root = os.path.dirname(args.save_path) or "."
            output_video = os.path.join(batch_output_root, item.get("output_video", videoname))
