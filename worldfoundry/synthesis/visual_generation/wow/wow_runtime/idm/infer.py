            ...
            with h5py.File(h5_file, "r") as f:
                ...
                with torch.no_grad():
                    for i in range(1, num_actions + 1):
                        ...
                        all_results.append(
                            {"file": os.path.basename(h5_file), "frame_index": i, "arm_joints": pred_action}
                        )
...
def main():
    ...
    parser.add_argument("--h5_files", type=str, nargs="+", required=True, help="List of input h5 file paths")
    parser.add_argument("--model_path", type=str, required=True, help="Path to model checkpoint")
    parser.add_argument(
        "--output_path",
        type=str,
        default=os.environ.get("INFER_OUTPUT", "simple_inference_results.json"),
        help="Output JSON file path (can be set via INFER_OUTPUT env var)",
    )
    parser.add_argument(
        "--device", type=str, default="cuda", help="CUDA computation device, for example cuda or cuda:0"
    )
    parser.add_argument(
        "--max_actions", type=int, default=None, help="Maximum number of actions to process per H5 file"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="dinov2_flow",
        choices=["dinov2", "dinov2_state", "dinov2_flow", "resnet"],
