...
class HeliosScheduler(SchedulerMixin, ConfigMixin):
    ...
    def init_sigmas_for_each_stage(self):
        ...
        schedule_steps = self.config.num_train_timesteps
        ...
        for i_s in range(stages):
            ...
            start_indice = int(stage_range[i_s] * schedule_steps)
            start_indice = max(start_indice, 0)
            end_indice = int(stage_range[i_s + 1] * schedule_steps)
            end_indice = min(end_indice, schedule_steps)
            start_sigma = self.sigmas[start_indice].item()
            end_sigma = self.sigmas[end_indice].item() if end_indice < schedule_steps else 0.0
        ...
        for i_s in range(stages):
            ...
            # timestep_max = self.timesteps[int(timestep_ratio[0] * schedule_steps)]
            timestep_max = min(self.timesteps[int(timestep_ratio[0] * schedule_steps)], 999)
            timestep_min = self.timesteps[min(int(timestep_ratio[1] * schedule_steps), schedule_steps - 1)]
            timesteps = np.linspace(timestep_max, timestep_min, schedule_steps + 1)
            self.timesteps_per_stage[i_s] = (
                timesteps[:-1] if isinstance(timesteps, torch.Tensor) else torch.from_numpy(timesteps[:-1])
            )
            stage_sigmas = np.linspace(0.999, 0, schedule_steps + 1)
