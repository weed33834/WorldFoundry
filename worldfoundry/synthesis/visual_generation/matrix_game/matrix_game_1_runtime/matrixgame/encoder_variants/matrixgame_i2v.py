...
class TextEncoder(nn.Module):
    ...
    @staticmethod
    def _expand_image_tokens_for_llava(
        input_ids: torch.Tensor,
        attention_mask: Optional[torch.Tensor],
        image_token_id: int,
        image_token_count: int,
    ) -> tuple[torch.Tensor, Optional[torch.Tensor]]:
        """Match older Llava merge semantics expected by MatrixGame checkpoints."""
        ...
        for batch_idx, row in enumerate(input_ids):
            id_chunks = []
            mask_chunks = [] if attention_mask is not None else None
            for token_idx, token in enumerate(row):
                repeat_count = image_token_count if int(token.item()) == image_token_id else 1
                id_chunks.append(token.repeat(repeat_count))
                if attention_mask is not None:
                    mask_chunks.append(attention_mask[batch_idx, token_idx].repeat(repeat_count))
            expanded_ids.append(torch.cat(id_chunks))
            if attention_mask is not None:
                expanded_masks.append(torch.cat(mask_chunks))

        expanded_input_ids = torch.stack(expanded_ids)
        expanded_attention_mask = torch.stack(expanded_masks) if attention_mask is not None else None
    ...
    def encode(
        ...
        if not self.i2v_mode:
            ...
            model_input_ids = batch_encoding["input_ids"].to(device)
            ...
            if image_token_id is not None:
                ...
                image_token_count = (prompt_template or {}).get("image_emb_len", 576)
                model_input_ids, model_attention_mask = self._expand_image_tokens_for_llava(
                    model_input_ids,
