...
def split_ctrl_world_latents(latents, rows=3, cols=1):
    batch, frames, channels, merged_height, merged_width = latents.shape
    if merged_height % rows != 0 or merged_width % cols != 0:
        raise ValueError(
            f"Latent shape {tuple(latents.shape)} cannot be split into a {rows}x{cols} grid."
        )
    height = merged_height // rows
    width = merged_width // cols
    return (
        latents.reshape(batch, frames, channels, rows, height, cols, width)
        .permute(0, 3, 5, 1, 2, 4, 6)
        .reshape(batch * rows * cols, frames, channels, height, width)
    )
