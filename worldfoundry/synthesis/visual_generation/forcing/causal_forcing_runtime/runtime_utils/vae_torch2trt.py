...
from eval_inputs import ShardingLMDBDataset

data_path = os.environ.get("WORLDFOUNDRY_VAE_LMDB_PATH", "").strip()
if not data_path:
    raise SystemExit(
        "Set WORLDFOUNDRY_VAE_LMDB_PATH to the LMDB shard directory before running vae_torch2trt."
    )
...
if hasattr(builder, "build_serialized_network"):          # TRT 10+
    ...
    engine_bytes = serialized_engine                      # keep for engine verification
else:                                                     # TRT ≤ 9
...
# ─────────────────────────────────────────────────────────
# 4️⃣  Quick verification with the brand-new engine
# ─────────────────────────────────────────────────────────
