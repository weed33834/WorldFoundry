...
def _runtime_asset_root():
    default_root = Path(__file__).resolve().parents[5] / "data" / "test_cases"
    return Path(os.environ.get("WORLDFOUNDRY_RUNTIME_ASSET_DIR", default_root))


def asset_path(*parts):
    root = Path(os.environ.get("WONDERJOURNEY_ASSET_DIR", _runtime_asset_root() / "wonderjourney"))
...
def load_inference_prompt_config(config):
    ...
    if missing:
        raise ValueError(
            "WonderJourney inference config must define "
            + ", ".join(required)
            + f"; missing {', '.join(missing)}."
        )
    yaml_data = {key: config.get(key) for key in required}
    for optional_key in ("background", "control_text"):
