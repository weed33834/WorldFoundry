        ...
        if disp_std > keep_threshold:
            ...
            if existing_mask is not None:  # When we are doing layer-inpainted image
                ...
                if intersection_mask.sum() > 20:  # This is a mask that includes regions from the initial outpainted image
                    existing_values = existing_disp[intersection_mask]
                    if existing_values.size == 0:
                        continue
                    median_val = np.percentile(existing_values, 50)  # Use the existing region disp values
...
def create_mask_generator_repvit():
    from worldfoundry.base_models.perception_core.segment.repvit_sam import (
        SamAutomaticMaskGenerator,
        sam_model_registry,
    )

    sam_checkpoint = _first_existing_ckpt("WonderWorld/repvit_sam.pt", "repvit_sam.pt")
    model_type = "repvit"
