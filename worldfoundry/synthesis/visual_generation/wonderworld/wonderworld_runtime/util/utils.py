...
def _runtime_asset_root():
    default_root = Path(__file__).resolve().parents[5] / "data" / "models" / "runtime_assets"
    return Path(os.environ.get("WORLDFOUNDRY_RUNTIME_ASSET_DIR", default_root))


def asset_path(*parts):
    root = Path(os.environ.get("WONDERWORLD_ASSET_DIR", _runtime_asset_root() / "wonderworld"))
    return root.joinpath(*parts)


def sky_image_dir(example_name):
    return Path("sky_img") / str(example_name)
...
def _resolve_asset_path(path):
    ...
    if path.is_absolute():
        return path
    candidate = asset_path(*path.parts)
    return candidate if candidate.exists() else path


def load_inference_prompt_config(config):
    required = ("image_filepath", "content_prompt", "style_prompt", "negative_prompt")
    missing = [key for key in required if not config.get(key)]
    if missing:
        raise ValueError(
            "WonderWorld inference config must define "
            + ", ".join(required)
            + f"; missing {', '.join(missing)}."
        )
    yaml_data = {key: config.get(key) for key in required}
    for optional_key in ("background", "control_text", "outdoor"):
        if optional_key in config:
            yaml_data[optional_key] = config.get(optional_key)
    yaml_data["image_filepath"] = str(_resolve_asset_path(yaml_data["image_filepath"]))
    return yaml_data
