...
def save_video(video, path, fps=10, save_gif=True):
    ...
    if _torchvision_write_video is not None:
        ...
        _torchvision_write_video(str(path), video, fps=fps, video_codec=video_codec, options=video_options)
    else:
        imageio.mimwrite(str(path), video_np, fps=fps, codec="libx264", quality=8)
    if not save_gif:
        return
