...
from groot_dreams.eval_inputs.schema import DatasetMetadata
from groot_dreams.eval_inputs.transform.base import ModalityTransform
...
class VideoCrop(VideoTransform):
    def get_transform(self, mode: Literal["train", "eval"] = "train") -> Callable:
        ...
        #     raise ValueError(f"Backend {self.backend} not supported")
        if self.backend == "torchvision":
            return self.__class__.center_crop
        else:
            raise ValueError(f"Backend {self.backend} not supported")
    ...
    def check_input(self, data: dict[str, Any]):
        ...
        #         raise ValueError(f"Backend {self.backend} not supported")
        #     assert (
        #             height == self.orig_height and width == self.orig_width
        #     ), f"Video {key} has invalid shape {height, width}, expected {self.orig_height, self.orig_width}"
    ...
    def center_crop(frames: torch.Tensor) -> torch.Tensor:
        ...
        if width / height > target_ratio:
            ...
            target_height = height
            target_width = width
        h_crop = (height - target_height) // 2
        w_crop = (width - target_width) // 2
        frames = frames[:, :, h_crop:h_crop + target_height, w_crop:w_crop + target_width]
        return frames
