"""Module for base_models -> perception_core -> segment -> sam3 -> model -> sam3_multiplex_detector_utils.py functionality."""
...
try:
    ...
    # Optional acceleration: install a compatible prebuilt torch_generic_nms wheel in the runtime image.
...
from worldfoundry.base_models.perception_core.segment.sam3.masks_ops import mask_iom
...
def _nms_masks_core_single(
    ...
    if GENERIC_NMS_AVAILABLE:
        ...
        logging.warning(
            "Falling back to CPU mask NMS implementation. Install a compatible prebuilt "
            "`torch_generic_nms` wheel for better performance."
        )
...
def perf_mask_iou(pred_masks: torch.Tensor, gt_masks: torch.Tensor) -> torch.Tensor:
    ...
    from worldfoundry.base_models.perception_core.segment.sam3.perflib.iou import pairwise_iou
...
def perf_mask_iom(pred_masks: torch.Tensor, gt_masks: torch.Tensor) -> torch.Tensor:
    ...
    """
    assert pred_masks.dtype == gt_masks.dtype == torch.bool
    from worldfoundry.base_models.perception_core.segment.sam3.perflib.iou import pairwise_iom
