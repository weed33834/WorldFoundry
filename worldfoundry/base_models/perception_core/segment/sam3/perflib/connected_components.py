...
# pyre-unsafe
"""Module for base_models -> perception_core -> segment -> sam3 -> perflib -> connected_components.py functionality."""
...
try:
    ...
    logging.debug(
        "cc_torch not found. Install a compatible prebuilt cc_torch wheel for better performance, "
        "or use the built-in CPU fallback."
    )
    HAS_CC_TORCH = False


def connected_components_cpu_single(values: torch.Tensor):
    """Connected components cpu single.
...
def connected_components(input_tensor: torch.Tensor):
    ...
    if input_tensor.is_cuda:
        if HAS_CC_TORCH:
            ...
            # triton fallback
            from worldfoundry.base_models.perception_core.segment.sam3.perflib.triton.connected_components import (
