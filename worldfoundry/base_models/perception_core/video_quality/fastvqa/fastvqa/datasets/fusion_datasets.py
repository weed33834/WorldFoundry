...
def _vread_with_skvideo(filename, height=1080, width=1920, inputdict=None):
    try:
        import skvideo.io
    except ImportError as exc:
        raise ImportError(
            "scikit-video is required only for FAST-VQA's skvideo fallback reader; "
            "install `scikit-video` or provide videos readable by decord."
        ) from exc
    return skvideo.io.vread(filename, height, width, inputdict=inputdict)
...
def get_spatial_and_temporal_samples(
    ...
    if video_path.endswith(".yuv"):
        ...
        ovideo = _vread_with_skvideo(video_path, 1080, 1920, inputdict={'-pix_fmt':'yuvj420p'})
...
class FusionDatasetK400(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if filename.endswith(".yuv"):
            ## This is only an adaptation to LIVE-Qualcomm
            video = _vread_with_skvideo(filename, 1080, 1920, inputdict={'-pix_fmt':'yuvj420p'})
