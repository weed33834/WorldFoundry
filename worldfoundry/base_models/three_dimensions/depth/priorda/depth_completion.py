...
"""Module for base_models -> three_dimensions -> depth -> priorda -> depth_completion.py functionality."""
...
class DepthCompletion(torch.nn.Module):
    ...
    def unify_format(
        ...
        Args:
            images: The images.
            sparse_depths: The sparse depths.
            sparse_masks: The sparse masks.
            cover_masks: The cover masks.
            prior_depths: The prior depths.
            geometric_depths: The geometric depths.
        """
    ...
    def calc_scale_shift(self, k_sparse_targets, k_pred_targets, currk_dists=None, knn=False):
        ...
        Args:
            k_sparse_targets: The k sparse targets.
            k_pred_targets: The k pred targets.
            currk_dists: The currk dists.
            knn: The knn.
        """
    ...
    def knn_aligns(
        ...
        # Use the official CUDA extension when it is available. The PyTorch fallback keeps
        # inference self-contained for environments where the extension was not compiled.
        knn_indices = None
        if os.environ.get("WORLDFOUNDRY_PRIORDA_USE_VIPE_EXT", "0") == "1":
            try:
                from .vipe_ext_loader import load_vipe_ext

                _C = load_vipe_ext()
                _, inds = _C.utils_ext.nearest_neighbours(y, x, K)
                knn_indices = inds.view(-1, K).long()  # [M, K]
            except (ImportError, ModuleNotFoundError, RuntimeError):
                knn_indices = None
        if knn_indices is None:
            knn_indices = self._torch_knn_indices(
                x=x,
                y=y,
                batch_x=_batch_x,
                batch_y=_batch_y,
    ...
    def _torch_knn_indices(
        *,
        ...
        """Helper function to torch knn indices.
        ...
        for batch_id in batch_y.unique(sorted=True):
            ...
            k = min(K, x_batch.shape[0])
            for y_chunk in torch.split(y_batch, chunk_size):
                dists = torch.cdist(y_chunk, x_batch)
                local = torch.topk(dists, k=k, dim=1, largest=False).indices
                global_indices = global_x[local]
                if k < K:
                    pad = global_indices[:, -1:].expand(-1, K - k)
                    global_indices = torch.cat([global_indices, pad], dim=1)
                indices.append(global_indices)
        if not indices:
            raise ValueError("PriorDA KNN fallback found no sparse/query points.")
