...
CONFIG_ROOT = resolve_data_path("models", "runtime", "configs", "depth_anything_v3", "configs")


def load_config(path: str, argv: List[str] = None) -> Union[DictConfig, ListConfig]:
    """
    Load a configuration. Will resolve inheritance.
    Supports both file paths and module paths (e.g., worldfoundry.base_models.three_dimensions.depth.depth_anything.depth_anything_v3.configs.giant).
    """
    ...
    if "." in path and "/" not in path and not path.endswith(".yaml"):
        # It's a module path. Support both the integrated package path and
        # upstream-style short paths such as `depth_anything_3.configs.da3-large`.
        package_parts = __package__.split(".")
        input_parts = path.split(".")

        if input_parts[: len(package_parts)] == package_parts:
            path_parts = input_parts[len(package_parts) :]
        elif "configs" in input_parts:
            path_parts = input_parts[input_parts.index("configs") + 1 :]
        else:
            path_parts = input_parts[1:]
