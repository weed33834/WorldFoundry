"""Module for base_models -> three_dimensions -> point_clouds -> pixelsplat_full -> src -> dataset -> view_sampler -> view_sampler_all.py functionality."""

from dataclasses import dataclass
from typing import Literal

import torch
from jaxtyping import Float, Int64
from torch import Tensor

from .view_sampler import ViewSampler


@dataclass
class ViewSamplerAllCfg:
    """View sampler all cfg implementation."""
    name: Literal["all"]


class ViewSamplerAll(ViewSampler[ViewSamplerAllCfg]):
    """View sampler all implementation."""
    def sample(
        self,
        scene: str,
        extrinsics: Float[Tensor, "view 4 4"],
        intrinsics: Float[Tensor, "view 3 3"],
        device: torch.device = torch.device("cpu"),
    ) -> tuple[
        Int64[Tensor, " context_view"],  # indices for context views
        Int64[Tensor, " target_view"],  # indices for target views
    ]:
        """Sample.

        Args:
            scene: The scene.
            extrinsics: The extrinsics.
            intrinsics: The intrinsics.
            device: The device.

        Returns:
            The return value.
        """
        v, _, _ = extrinsics.shape
        all_frames = torch.arange(v, device=device)
        return all_frames, all_frames

    @property
    def num_context_views(self) -> int:
        """Num context views.

        Returns:
            The return value.
        """
        return 0

    @property
    def num_target_views(self) -> int:
        """Num target views.

        Returns:
            The return value.
        """
        return 0
