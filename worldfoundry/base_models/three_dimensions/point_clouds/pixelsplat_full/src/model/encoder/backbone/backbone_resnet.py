"""Module for base_models -> three_dimensions -> point_clouds -> pixelsplat_full -> src -> model -> encoder -> backbone -> backbone_resnet.py functionality."""
...
class BackboneResnetCfg:
    """Backbone resnet cfg implementation."""
    name: Literal["resnet"]
    model: Literal[
...
class BackboneResnet(Backbone[BackboneResnetCfg]):
    """Backbone resnet implementation."""
    model: ResNet
    ...
    def __init__(self, cfg: BackboneResnetCfg, d_in: int) -> None:
        ...
        if cfg.model == "dino_resnet50":
            dino_hub = Path(__file__).resolve().parents[4] / "third_party" / "dino"
            self.model = torch.hub.load(str(dino_hub), "dino_resnet50", source="local", pretrained=False)
        else:
