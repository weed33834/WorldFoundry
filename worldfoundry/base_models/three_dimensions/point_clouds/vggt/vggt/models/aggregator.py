...
"""Module for base_models -> three_dimensions -> point_clouds -> vggt -> vggt -> models -> aggregator.py functionality."""
...
class Aggregator(nn.Module):
    ...
    def forward(self, images: torch.Tensor, cameras_possibly_zero=None) -> Tuple[List[torch.Tensor], int]:
        """
        Args:
            images (torch.Tensor): Input images with shape [B, S, 3, H, W], in range [0, 1].
                B: batch size, S: sequence length, 3: RGB channels, H: height, W: width
            cameras_possibly_zero (torch.Tensor, optional): Camera-conditioning tokens
                with shape [B, S, camera_tokens, embed_dim].
        ...
        if cameras_possibly_zero is not None:
            cameras_possibly_zero = cameras_possibly_zero.reshape(B * S, *cameras_possibly_zero.shape[2:])
