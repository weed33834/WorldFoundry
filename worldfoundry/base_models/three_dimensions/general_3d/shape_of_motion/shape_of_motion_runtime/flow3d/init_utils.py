"""Module for base_models -> three_dimensions -> general_3d -> shape_of_motion -> shape_of_motion_runtime -> flow3d -> init_utils.py functionality."""
...
try:
    import cupy as cp
except ModuleNotFoundError:  # WorldFoundry validation tests should not require RAPIDS.
    cp = None
...
def init_motion_params_with_procrustes(
    ...
    """Init motion params with procrustes.

    Args:
        tracks_3d: The tracks 3d.
        num_bases: The num bases.
        rot_type: The rot type.
        cano_t: The cano t.
        cluster_init_method: The cluster init method.
        min_mean_weight: The min mean weight.
        vis: The vis.
        port: The port.
    ...
    min_cluster_count = min(100, max(1, int(labels.numel()) // max(1, num_bases) // 2))
    ids = ids[counts >= min_cluster_count]
    if len(ids) == 0:
        ids = labels.unique()[: max(1, num_bases)]
    num_bases = len(ids)
...
def sample_initial_bases_centers(
    ...
    if cp is None or KMeans is None:
        return _sample_initial_bases_centers_cpu(mode, cano_t, tracks_3d, num_bases)

    # linearly interpolate missing 3d points
    xyz = cp.asarray(tracks_3d.xyz.detach())
    print(f"{xyz.shape=}")
    visibles = cp.asarray(tracks_3d.visibles.detach())
...
def _sample_initial_bases_centers_cpu(
    mode: str, cano_t: int, tracks_3d: TrackObservations, num_bases: int
):
    """Helper function to sample initial bases centers cpu.
    ...
    """
    means_canonical = tracks_3d.xyz[:, cano_t].clone()
    xyz = tracks_3d.xyz.detach().cpu().numpy()
    visibles = tracks_3d.visibles.detach().cpu().numpy().astype(bool)
    num_tracks = xyz.shape[0]
    num_bases = max(1, min(int(num_bases), num_tracks))
    xyz_interp = batched_interp_masked_np(xyz, visibles)
    velocities = xyz_interp[:, 1:] - xyz_interp[:, :-1]
    vel_dirs = velocities / (np.linalg.norm(velocities, axis=-1, keepdims=True) + 1e-5)
    vel_dirs = vel_dirs.reshape((num_tracks, -1))

    labels_np: np.ndarray
    use_sklearn = os.getenv("WORLDFOUNDRY_SHAPE_OF_MOTION_USE_SKLEARN", "").strip().lower() in {"1", "true", "yes", "on"}
    if mode == "hdbscan" and use_sklearn:
        try:
            from sklearn.cluster import HDBSCAN as SkHDBSCAN

            labels_np = SkHDBSCAN(min_cluster_size=max(2, min(20, num_tracks // 2))).fit_predict(vel_dirs)
            if np.all(labels_np < 0):
                labels_np = np.arange(num_tracks) % num_bases
            labels_np = np.where(labels_np < 0, 0, labels_np)
        except Exception:
            labels_np = _kmeans_or_bucket_labels(vel_dirs, num_bases)
    else:
        labels_np = _kmeans_or_bucket_labels(vel_dirs, num_bases)

    labels = torch.as_tensor(labels_np, dtype=torch.long, device=means_canonical.device)
    unique_ids = labels.unique()
    sampled = []
    for cluster_id in unique_ids:
        cluster_points = means_canonical[labels == cluster_id]
        sampled.append(cluster_points.median(dim=0).values)
    sampled_centers = torch.stack(sampled, dim=0)[None]
    num_bases = sampled_centers.shape[1]
    print("number of {} clusters: ".format(mode), num_bases)
    return sampled_centers, num_bases, labels


def _kmeans_or_bucket_labels(values: np.ndarray, num_bases: int) -> np.ndarray:
    """Helper function to kmeans or bucket labels.
    ...
    Returns:
        The return value.
    """
    if os.getenv("WORLDFOUNDRY_SHAPE_OF_MOTION_USE_SKLEARN", "").strip().lower() in {"1", "true", "yes", "on"}:
        try:
            from sklearn.cluster import KMeans as SkKMeans

            return SkKMeans(n_clusters=num_bases, n_init=1, random_state=0).fit_predict(values)
        except Exception:
            pass
    return np.arange(values.shape[0], dtype=np.int64) % max(1, num_bases)


def interp_masked_np(vals: np.ndarray, mask: np.ndarray, pad: int = 1) -> np.ndarray:
    """Interp masked np.
    ...
    for b in range(B):
        ...
        if len(valid) == 0:
            continue
        for d in range(D):
            out[b, :, d] = np.interp(xs, valid, vals[b, valid, d])
    return out.reshape((B, M, *sh))


def batched_interp_masked_np(vals: np.ndarray, mask: np.ndarray) -> np.ndarray:
    """Batched interp masked np.
    ...
    Returns:
        The return value.
    """
    return interp_masked_np(vals, mask)


def interp_masked(vals, mask, pad: int = 1):
    """
...
def batched_interp_masked(vals, mask, batch_num: int = 4096, batch_time: int = 64):
    """Batched interp masked.

    Args:
        vals: The vals.
        mask: The mask.
        batch_num: The batch num.
        batch_time: The batch time.
    """
    assert mask.ndim == 2
    B, M = mask.shape
