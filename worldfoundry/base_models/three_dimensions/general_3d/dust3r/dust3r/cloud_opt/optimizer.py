"""Module for base_models -> three_dimensions -> general_3d -> dust3r -> dust3r -> cloud_opt -> optimizer.py functionality."""
...
def _first_existing_path(*paths):
    """Helper function to first existing path."""
    for path in paths:
...
from dust3r.utils.optical_flow import load_raft
from worldfoundry.base_models.perception_core.segment.sam2.build_sam import build_sam2_video_predictor
_CKPT_ROOT = _find_ckpt_root()
sam2_checkpoint = _first_existing_path(
    None if _CKPT_ROOT is None else _CKPT_ROOT / "sam2.1-hiera-large" / "sam2.1_hiera_large.pt",
)
...
def smooth_L1_loss_fn(estimate, gt, mask, beta=1.0, per_pixel_thre=50.):
    ...
    Args:
        ...
        mask: The mask.
        beta: The beta.
        per_pixel_thre: The per pixel thre.
    """
...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def get_flow(self, sintel_ckpt=False): #TODO: test with gt flow
        ...
        flow_net = load_raft() if sintel_ckpt else load_raft("Tartan-C-T-TSKH-spring540x960-M.pth")
    ...
    def _check_all_imgs_are_selected(self, msk):
        """Helper function to check all imgs are selected.
    ...
    def preset_pose(self, known_poses, pose_msk=None, requires_grad=False):  # cam-to-world
        ...
        Args:
            known_poses: The known poses.
            pose_msk: The pose msk.
    ...
    def preset_intrinsics(self, known_intrinsics, msk=None):
        ...
        Args:
            known_intrinsics: The known intrinsics.
            msk: The msk.
        """
    ...
    def preset_principal_point(self, known_pp, msk=None):
        """Preset principal point.

        Args:
            known_pp: The known pp.
            msk: The msk.
        """
        self._check_all_imgs_are_selected(msk)
    ...
    def _get_msk_indices(self, msk):
        """Helper function to get msk indices.
    ...
    def get_known_focal_mask(self):
        """Get known focal mask."""
        return torch.tensor([not (p.requires_grad) for p in self.im_focals])

    def _set_principal_point(self, idx, pp, force=False):
        """Helper function to set principal point.
    ...
    def _set_depthmap_batch(self, idx, depth, force=False):
        """Helper function to set depthmap batch.
    ...
    def _set_depthmap_non_batch(self, idx, depth, force=False):
        """Helper function to set depthmap non batch.
    ...
    def _set_depthmap(self, idx, depth, force=False):
        """Helper function to set depthmap.
    ...
    def preset_depthmap(self, known_depthmaps, msk=None, requires_grad=False):
        ...
        Args:
            known_depthmaps: The known depthmaps.
            msk: The msk.
            requires_grad: The requires grad.
        """
        self._check_all_imgs_are_selected(msk)
    ...
    def _set_init_depthmap(self):
        """Helper function to set init depthmap."""
...
def _fast_depthmap_to_pts3d(depth, pixel_grid, focal, pp):
    """Helper function to fast depthmap to pts3d.
