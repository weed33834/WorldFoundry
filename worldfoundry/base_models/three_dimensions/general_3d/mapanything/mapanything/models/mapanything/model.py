...
class MapAnything(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        # The released MapAnything state dict already contains the complete
        # DINOv2 encoder.  Initializing another pretrained encoder here would
        # trigger a torch-hub network download before those weights are
        # overwritten, so inference always builds the in-tree architecture
        # without external weights.
        encoder_config_copy["torch_hub_pretrained"] = False
    ...
    def _load_pretrained_weights(self):
        ...
        if self.pretrained_checkpoint_path is not None:
            ...
            if not isinstance(checkpoint, Mapping):
                raise TypeError(
                    "MapAnything checkpoint must be a mapping containing a 'model' state dict."
                )
            model_state = checkpoint.get("model")
            if not isinstance(model_state, Mapping):
                raise TypeError(
                    "MapAnything checkpoint field 'model' must be a tensor mapping."
                )
            invalid_entries = [
                key
            ...
            if invalid_entries:
                raise TypeError(
                    "MapAnything checkpoint contains non-tensor state entries: "
                    f"{invalid_entries[:5]}"
                )
            if not self.load_specific_pretrained_submodules:
                ...
                print(self.load_state_dict(model_state))
            else:
                print(
