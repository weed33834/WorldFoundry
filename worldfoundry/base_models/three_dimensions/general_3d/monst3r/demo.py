...
def _load_runtime_deps():
    ...
    global torch, np, inference, AsymmetricCroCo3DStereo, make_pairs
    global load_images, load_prev_video_results, rgb, enlarge_seg_masks
    global to_numpy, global_aligner, GlobalAlignerMode
    global convert_scene_output_to_glb, get_dynamic_mask_from_pairviewer, pl, cv2
    ...
    from dust3r.cloud_opt import global_aligner as _global_aligner, GlobalAlignerMode as _GlobalAlignerMode
    from dust3r.image_pairs import make_pairs as _make_pairs
    from dust3r.inference import inference as _inference
    from dust3r.model import AsymmetricCroCo3DStereo as _AsymmetricCroCo3DStereo
    from dust3r.utils.device import to_numpy as _to_numpy
    from dust3r.utils.image import enlarge_seg_masks as _enlarge_seg_masks
    from dust3r.utils.image import load_images as _load_images
    from dust3r.utils.image import load_prev_video_results as _load_prev_video_results
    from dust3r.utils.image import rgb as _rgb
    from dust3r.utils.viz_demo import convert_scene_output_to_glb as _convert_scene_output_to_glb
    from dust3r.utils.viz_demo import get_dynamic_mask_from_pairviewer as _get_dynamic_mask_from_pairviewer
    ...
    AsymmetricCroCo3DStereo = _AsymmetricCroCo3DStereo
    make_pairs = _make_pairs
    load_images = _load_images
    load_prev_video_results = _load_prev_video_results
    rgb = _rgb
    enlarge_seg_masks = _enlarge_seg_masks
    to_numpy = _to_numpy
    global_aligner = _global_aligner
    GlobalAlignerMode = _GlobalAlignerMode
    convert_scene_output_to_glb = _convert_scene_output_to_glb
    get_dynamic_mask_from_pairviewer = _get_dynamic_mask_from_pairviewer
    pl.ion()
...
def get_args_parser():
    ...
    parser.add_argument("--input_dir", "--input", dest="input_dir", type=str,
                        help="Path to input images directory or video", default=None)
    parser.add_argument("--seq_name", type=str, help="Sequence name for evaluation", default='NULL')
    parser.add_argument("--save_name", type=str, help="Base name for the exported GLB", default=None)
    ...
    parser.add_argument('--niter', type=int, default=300, help='Global alignment iterations for non-interactive inference')
    parser.add_argument('--scenegraph_type', type=str, default='swinstride',
                        choices=['complete', 'swin', 'oneref', 'swinstride', 'swin2stride'],
                        help='Scene graph used for non-interactive inference')
    parser.add_argument('--winsize', type=int, default=5, help='Window size for swin scene graph modes')
    parser.add_argument('--refid', type=int, default=0, help='Reference image id for oneref scene graph mode')
    parser.add_argument('--min_conf_thr', type=float, default=1.1, help='Minimum confidence threshold for GLB export')
    parser.add_argument('--cam_size', type=float, default=0.05, help='Camera frustum size in the exported GLB')
    parser.add_argument('--temporal_smoothing_weight', type=float, default=0.01, help='Temporal smoothing weight')
    parser.add_argument('--translation_weight', type=str, default='1.0', help='Translation regularization weight')
    parser.add_argument('--flow_loss_weight', type=float, default=0.01, help='Flow loss weight')
    parser.add_argument('--flow_loss_start_iter', type=float, default=0.1, help='Flow loss start ratio')
    parser.add_argument('--flow_loss_threshold', type=float, default=25, help='Flow loss threshold')
    parser.add_argument('--skip_pair_dynamic_mask', action='store_true', default=False,
                        help='Skip optional RAFT dynamic-mask gallery export for two-frame inputs')
...
def get_reconstructed_scene(args, outdir, model, device, silent, image_size, filelist, schedule, niter, min_conf_thr,
                            ...
                            save_name=None, skip_pair_dynamic_mask=False):
    """
    ...
    outfile = get_3D_model_from_scene(save_folder, silent, scene, min_conf_thr, as_pointcloud, mask_sky,
                            clean_depth, transparent_cams, cam_size, show_cam, save_name=save_name)
    ...
    if len(rgbimg) == 2 and rgbimg[0].shape == rgbimg[1].shape and not skip_pair_dynamic_mask:
    ...
    if args.input_dir is not None:
        ...
        if args.real_time:
            ...
            scene, outfile, imgs = recon_fun(
                ...
                skip_pair_dynamic_mask=args.skip_pair_dynamic_mask,
            )
