...
# --------------------------------------------------------
"""Module for base_models -> three_dimensions -> general_3d -> monst3r -> dust3r -> cloud_opt -> base_opt.py functionality."""
...
def _save_trajectory_tum_format(traj, path):
    from dust3r.utils.vo_eval import save_trajectory_tum_format

    return save_trajectory_tum_format(traj, path)
...
class BasePCOptimizer (nn.Module):
    ...
    def _init_from_views(self, view1, view2, pred1, pred2,
                         ...
                         empty_cache=False,
                         verbose=True):
        """Helper function to init from views.
        ...
        Args:
            ...
            thr_for_init_conf: The thr for init conf.
            base_scale: The base scale.
            allow_pw_adaptors: The allow pw adaptors.
            pw_break: The pw break.
            rand_pose: The rand pose.
            empty_cache: The empty cache.
            verbose: The verbose.
        """
        super().__init__()
    ...
    def _compute_img_conf(self, pred1_conf, pred2_conf):
        """Helper function to compute img conf.

        Args:
            pred1_conf: The pred1 conf.
            pred2_conf: The pred2 conf.
        """
    ...
    def save_tum_poses(self, path):
        ...
        _save_trajectory_tum_format(traj, path)
    ...
    def compute_global_alignment(self, init=None, save_score_path=None, save_score_only=False, niter_PnP=10, **kw):
        ...
        Args:
            init: The init.
            save_score_path: The save score path.
            save_score_only: The save score only.
            niter_PnP: The niter pnp.
        """
        if init is None:
            pass
    ...
    def show(self, show_pw_cams=False, show_pw_pts3d=False, cam_size=None, **kw):
        ...
        Args:
            show_pw_cams: The show pw cams.
            show_pw_pts3d: The show pw pts3d.
...
def global_alignment_loop(net, lr=0.01, niter=300, schedule='cosine', lr_min=1e-3, temporal_smoothing_weight=0, depth_map_save_dir=None):
    ...
    Args:
        ...
        lr_min: The lr min.
        temporal_smoothing_weight: The temporal smoothing weight.
        depth_map_save_dir: The depth map save dir.
    """
...
def global_alignment_iter(net, cur_iter, niter, lr_base, lr_min, optimizer, schedule, temporal_smoothing_weight=0):
    """Global alignment iter.

    Args:
        net: The net.
        cur_iter: The cur iter.
        niter: The niter.
        lr_base: The lr base.
        lr_min: The lr min.
