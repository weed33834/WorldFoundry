...
from .media import Image, Video, _load_video, extract_media
...
def fetch_video(ele: dict, return_video_sample_fps: bool = False):
    video = ele.get("video") or ele.get("path") or ele.get("video_url")
    if video is None:
        raise ValueError(f"Video entry must contain `video`, `path`, or `video_url`: {ele}")
    if not isinstance(video, str):
        raise TypeError(f"VILA video input must be a local path or URL string, got {type(video)!r}")
    video_path = fetch_image_url_or_fpath(video) if video.startswith(("http://", "https://", "file://")) else video
    num_frames = int(ele.get("num_frames", ele.get("frames", 8)))
    frames = _load_video(video_path, num_frames=num_frames)
    sample_fps = float(ele.get("fps", 0.0))
...
class VILAProcessor(ProcessorMixin):
    ...
    def process_vision_info(
        ...
        """
        vision_infos = VILAProcessor.extract_vision_info(conversations)
