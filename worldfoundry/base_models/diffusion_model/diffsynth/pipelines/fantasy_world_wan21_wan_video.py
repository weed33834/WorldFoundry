"""Module for base_models -> diffusion_model -> diffsynth -> pipelines -> fantasy_world_wan21_wan_video.py functionality."""

import types
from ..models.fantasy_world_wan21_model_manager import ModelManager
from ..models.fantasy_world_wan21_wan_video_dit import WanModel
from ..models.wan_video_text_encoder import WanTextEncoder
from ..models.fantasy_world_wan21_wan_video_vae import WanVideoVAE
...
from ..models.fantasy_world_wan21_wan_video_dit import RMSNorm, sinusoidal_embedding_1d
from ..models.fantasy_world_wan21_wan_video_vae import RMS_norm, CausalConv3d, Upsample
...
class WanVideoPipeline(BasePipeline):
    ...
    def fetch_models(self, model_manager: ModelManager):
        ...
        self.image_encoder = model_manager.fetch_model(
            "wan_video_image_encoder")
        self.motion_controller = model_manager.fetch_model(
    ...
    def encode_prompt(self, prompt, positive=True):
        ...
        prompt_emb = self.prompter.encode_prompt(
            prompt, positive=positive, device=self.device)
    ...
    def encode_image(
        ...
        if end_image is not None:
            end_image = self.preprocess_image(
                end_image.resize(
                    (width, height))).to(
                self.device)
        ...
        y = self.vae.encode([vae_input.to(dtype=self.torch_dtype,
                                          device=self.device)],
                            device=self.device,
                            tiled=tiled,
                            tile_size=tile_size,
    ...
    def encode_control_video(
        self, control_video, tiled=True, tile_size=(
            34, 34), tile_stride=(
            18, 16)):
        ...
        control_video = torch.stack(
            control_video, dim=2).to(
            dtype=self.torch_dtype, device=self.device)
        latents = self.encode_video(
            control_video,
            tiled=tiled,
            tile_size=tile_size,
            tile_stride=tile_stride).to(
    ...
    def prepare_reference_image(self, reference_image, height, width):
        ...
        if reference_image is not None:
            ...
            reference_image = torch.stack(
                reference_image, dim=2).to(
                dtype=self.torch_dtype, device=self.device)
            reference_latents = self.vae.encode(
                reference_image, device=self.device)
            return {"reference_latents": reference_latents}
        else:
            return {}

    def prepare_controlnet_kwargs(
        self,
        control_video,
        num_frames,
        height,
        ...
        if control_video is not None:
            control_latents = self.encode_control_video(
                control_video, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
            if clip_feature is None or y is None:
                clip_feature = torch.zeros(
                    (1, 257, 1280), dtype=self.torch_dtype, device=self.device)
                y = torch.zeros(
                    (1,
                     16,
                     (num_frames - 1) // 4 + 1,
    ...
    def encode_video(
        self, input_video, tiled=True, tile_size=(
            34, 34), tile_stride=(
            18, 16)):
        """Encode video.
    ...
    def decode_video(
        self, latents, tiled=True, tile_size=(
            34, 34), tile_stride=(
            18, 16)):
        """Decode video.
        ...
        if vace_video is not None or vace_mask is not None or vace_reference_image is not None:
            ...
            if vace_video is None:
                vace_video = torch.zeros(
                    (1, 3, num_frames, height, width), dtype=self.torch_dtype, device=self.device)
            else:
                vace_video = self.preprocess_images(vace_video)
                vace_video = torch.stack(vace_video, dim=2).to(
                    dtype=self.torch_dtype, device=self.device)
            ...
            vace_mask_latents = rearrange(
                vace_mask[0, 0], "T (H P) (W Q) -> 1 (P Q) T H W", P=8, Q=8)
            ...
            if vace_reference_image is None:
                ...
                vace_reference_image = self.preprocess_images(
                    [vace_reference_image])
                vace_reference_image = torch.stack(
                    vace_reference_image, dim=2).to(
                    dtype=self.torch_dtype, device=self.device)
                vace_reference_latents = self.encode_video(
                    vace_reference_image,
                    tiled=tiled,
                    tile_size=tile_size,
                    tile_stride=tile_stride).to(
    ...
    def __call__(
        ...
        if input_video is not None:
            ...
            input_video = torch.stack(
                input_video, dim=2).to(
                dtype=self.torch_dtype, device=self.device)
            latents = self.encode_video(
                input_video, **tiler_kwargs).to(dtype=self.torch_dtype, device=self.device)
        ...
        # Reference image
        reference_image_kwargs = self.prepare_reference_image(
            reference_image, height, width)

        # ControlNet
        if control_video is not None:
            self.load_models_to_device(["image_encoder", "vae"])
            image_emb = self.prepare_controlnet_kwargs(
                control_video, num_frames, height, width, **image_emb, **tiler_kwargs)
    ...
    **kwargs,
    ...
    if reference_latents is not None:
        reference_latents = dit.ref_conv(
            reference_latents[:, :, 0]).flatten(2).transpose(1, 2)
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            if vace_context is not None and block_id in vace.vace_layers_mapping:
                x = x + \
                    vace_hints[vace.vace_layers_mapping[block_id]] * vace_scale
        if tea_cache is not None:
            tea_cache.store(x)
