"""Module for base_models -> diffusion_model -> diffsynth -> pipelines -> wan_video_pusa.py functionality."""
...
class WanVideoPusaPipeline(BasePipeline):
    """Wan video pusa pipeline implementation."""
    ...
    def __call__(
        ...
        Args:
            ...
            visualize_attention: The visualize attention.
    ...
    **kwargs,
    ...
    if dit.has_image_input:
        if y is None or clip_feature is None:
            raise ValueError("Wan PUSA image-input models require both `y` and `clip_feature`.")
    ...
    if add_condition is not None:
        add_condition = add_condition.to(dtype=x.dtype, device=x.device)
        if add_condition.shape != x.shape:
            raise RuntimeError(
                "Wan PUSA add_condition shape mismatch: "
                f"expected {tuple(x.shape)}, got {tuple(add_condition.shape)}"
            )
        x = x + add_condition
    ...
    if use_unified_sequence_parallel:
        if dist.is_initialized() and dist.get_world_size() > 1:
            ...
            pad_shape = x_chunks[0].shape[1] - x_chunks[-1].shape[1]
            x_chunks = [
                torch.nn.functional.pad(chunk, (0, 0, 0, x_chunks[0].shape[1] - chunk.shape[1]), value=0)
                for chunk in x_chunks
            ]
            t_chunks = torch.chunk(t, sp_size, dim=1)
            t_chunks = [
                torch.nn.functional.pad(chunk, (0, 0, 0, t_chunks[0].shape[1] - chunk.shape[1]), value=0)
                for chunk in t_chunks
            ]
            t_mod_chunks = torch.chunk(t_mod, sp_size, dim=2)
            t_mod_chunks = [
                torch.nn.functional.pad(
                    chunk,
                    (0, 0, 0, t_mod_chunks[0].shape[2] - chunk.shape[2]),
                    value=0,
                )
                for chunk in t_mod_chunks
            ]
            x = x_chunks[sp_rank]
            t = t_chunks[sp_rank]
            t_mod = t_mod_chunks[sp_rank]
    if tea_cache_update:
