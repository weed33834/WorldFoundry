"""Module for base_models -> diffusion_model -> diffsynth -> pipelines -> longvie_wan_video_new.py functionality."""
...
from ..models.longvie_wan_video_dit import WanModel, RMSNorm, sinusoidal_embedding_1d
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.inv_scheduler = FlowMatchScheduler(shift=5, sigma_min=0.0, extra_one_step=True, inverse_timesteps=True)
    ...
    def regenerate(
        ...
        Args:
            ...
            tea_cache_model_id: The tea cache model id.
            progress_bar_cmd: The progress bar cmd.
            inverse_step: The inverse step.
            save_dir: The save dir.
        """
        # Scheduler
        self.scheduler.set_timesteps(num_inference_steps, denoising_strength=denoising_strength, shift=sigma_shift)
        self.inv_scheduler.set_timesteps(num_inference_steps, denoising_strength=denoising_strength, shift=sigma_shift)
        ...
        # for progress_id, timestep in enumerate(progress_bar_cmd(self.inv_scheduler.timesteps[:inverse_step])):
        #     # Timestep
        #     timestep = timestep.unsqueeze(0).to(dtype=self.torch_dtype, device=self.device)
        ...
        #     # Scheduler
        #     inputs_shared["latents"] = self.inv_scheduler.step(noise_pred, self.inv_scheduler.timesteps[progress_id], inputs_shared["latents"])
        #     self.load_models_to_device(['vae'])
        #     video = self.vae.decode(inputs_shared["latents"], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        #     video = self.vae_output_to_video(video)
        #     self.load_models_to_device([])
            
        #     video[0].save("add_noised_{}/add_noise_{}.png".format(inverse_step,progress_id))

        noise = torch.randn_like(inputs_shared["latents"]).to(inputs_shared["latents"].dtype).to(inputs_shared["latents"].device)
        gt_latent = inputs_shared["latents"].clone()
        for inverse_step in [1, 5, 8, 10, 12]:
            inputs_shared["latents"] = self.scheduler.add_noise(gt_latent, noise, timestep=self.scheduler.timesteps[-inverse_step:][0])
            for progress_id, timestep in enumerate(self.scheduler.timesteps[-inverse_step:]):
                timestep = timestep.unsqueeze(0).to(dtype=self.torch_dtype, device=self.device)
                ...
                # Scheduler
                inputs_shared["latents"] = self.scheduler.step(noise_pred, self.scheduler.timesteps[-inverse_step:][progress_id], inputs_shared["latents"])
            
            video = self.vae.decode(inputs_shared["latents"], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
            video = self.vae_output_to_video(video)

            # video[0].save(f"{save_dir}/inv_step_{inverse_step}.png")
            save_video(video, f"{save_dir}/inv_step_{inverse_step}.mp4", fps=16, quality=10)
        return video
    ...
    def re_ae(
        ...
        for i in range(10):            
            ...
            video[0].save("encode_{}/recon_{}.png".format(10,i))
            video = self.preprocess_video(video)
            latent = self.vae.encode(video, device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=torch.bfloat16, device=video.device)
            # video = self.vae.encode(video, device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        self.load_models_to_device([])
        return video
...
class WanVideoPostUnit_AnimateVideoSplit(PipelineUnit):
    """Wan video post unit animate video split implementation."""
    def __init__(self):
        """Init."""
...
class WanVideoPostUnit_AnimatePoseLatents(PipelineUnit):
    """Wan video post unit animate pose latents implementation."""
    def __init__(self):
        """Init."""
        super().__init__(
...
class WanVideoPostUnit_AnimateFacePixelValues(PipelineUnit):
    """Wan video post unit animate face pixel values implementation."""
    def __init__(self):
        """Init."""
        super().__init__(take_over=True)
...
class WanVideoPostUnit_AnimateInpaint(PipelineUnit):
    """Wan video post unit animate inpaint implementation."""
    def __init__(self):
        """Init."""
        super().__init__(
