...
Orchestrates the full inference flow: text encoding -> noise initialization ->
iterative denoising (with action conditioning) -> VAE decoding -> video output.

Built on the Wan2.2 video generation pipeline with added support for
keyboard_action and mouse_action inputs passed to the SCOPE DiT.
"""
import types
...
from worldfoundry.core.device import get_device_name, get_device_type
...
from ..models.scope_wan_video_dit import WanModel, sinusoidal_embedding_1d
from ..models.scope_wan_video_dit_s2v import rope_precompute
from ..models.scope_wan_video_text_encoder import WanTextEncoder, HuggingfaceTokenizer
from ..models.scope_wan_video_vae import WanVideoVAE
from ..models.scope_wan_video_image_encoder import WanImageEncoder
from ..models.scope_wan_video_vace import VaceWanModel
from ..models.scope_wan_video_motion_controller import WanMotionControllerModel
from ..models.scope_wan_video_animate_adapter import WanAnimateAdapter
from ..models.wan_video_mot import MotWanModel
from ..models.scope_wav2vec import WanS2VAudioEncoder
...
class WanVideoPipeline(BasePipeline):
    ...
    def enable_usp(self):
        """Enable usp."""
        from worldfoundry.core.attention.scope_xdit_context_parallel import get_sequence_parallel_world_size, usp_attn_forward, usp_dit_forward
    ...
    def from_pretrained(
        ...
        if use_usp:
            from worldfoundry.core.attention.scope_xdit_context_parallel import initialize_usp
    ...
    def __call__(
        ...
        Args:
            ...
            vap_video: The vap video.
            vap_prompt: The vap prompt.
            negative_vap_prompt: The negative vap prompt.
            seed: The seed.
        ...
        inputs_shared = {
            ...
            "vap_video": vap_video, 
            "mouse_action": mouse_action, "keyboard_action": keyboard_action
        }
        ...
        # VACE reference frame removal (post-denoising adjustment).
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    Args:
        ...
        vace_scale: The vace scale.
        vap_hidden_state: The vap hidden state.
        vap_clip_feature: The vap clip feature.
        context_vap: The context vap.
    ...
    if hasattr(dit, 'freqs_mouse') and dit.freqs_mouse is not None:
        freqs_mouse = dit.freqs_mouse[:f]
        freqs_mouse = freqs_mouse.view(f, 1, -1)
        freqs_mouse = freqs_mouse.to(x.device)
    else:
        # Default to None if freqs_mouse is not available on the model.
        freqs_mouse = None
    ...
    if use_unified_sequence_parallel:
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            if vap is not None and block_id in vap.mot_layers_mapping:
                ...
                if freqs_mouse is not None:
                    x = gradient_checkpoint_forward(
                        block,
                        use_gradient_checkpointing,
                        use_gradient_checkpointing_offload,
                        x, context, t_mod, freqs, f, h, w,
                        mouse_action=mouse_action, keyboard_action=keyboard_action, freqs_mouse=freqs_mouse
                    )
                else:
                    x = gradient_checkpoint_forward(
                        block,
