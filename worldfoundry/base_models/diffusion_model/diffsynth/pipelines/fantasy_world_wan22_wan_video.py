"""Module for base_models -> diffusion_model -> diffsynth -> pipelines -> fantasy_world_wan22_wan_video.py functionality."""

import types
from ..models.fantasy_world_wan22_model_manager import ModelManager
from ..models.fantasy_world_wan22_wan_video_dit import WanModel
from ..models.wan_video_text_encoder import WanTextEncoder
from ..models.fantasy_world_wan22_wan_video_vae import WanVideoVAE
...
from ..models.fantasy_world_wan22_wan_video_dit import RMSNorm, sinusoidal_embedding_1d
from ..models.fantasy_world_wan22_wan_video_vae import RMS_norm, CausalConv3d, Upsample
...
class WanVideoPipeline(BasePipeline):
    ...
    def encode_image(self, image, end_image, num_frames, height, width, mid_images_list = None, mid_images_id_list = None, tiled=False, tile_size=(34, 34), tile_stride=(18, 16)):
        """Encode image.
        ...
        Args:
            ...
            height: The height.
            width: The width.
            mid_images_list: The mid images list.
            mid_images_id_list: The mid images id list.
            tiled: The tiled.
            tile_size: The tile size.
            tile_stride: The tile stride.
        """
        image = self.preprocess_image(image.resize((width, height))).to(self.device)
        if self.image_encoder is not None and self.dit.require_clip_embedding:
        ...
        if end_image is not None:
            ...
            if self.image_encoder is not None and self.dit.require_clip_embedding and self.dit.has_image_pos_emb:
        ...
        if mid_images_list is not None and mid_images_id_list is not None:
            for mid_image, mid_image_id in zip(mid_images_list, mid_images_id_list):
                mid_image = self.preprocess_image(mid_image.resize((width, height))).to(self.device)
    ...
    def __call__(
        ...
        # Encode image
        if input_image is None or not self.dit.require_vae_embedding:
            image_emb = {}
        else:
    ...
    def one_step(
        ...
        Args:
            ...
            mid_images_id_list: The mid images id list.
            progress_bar_st: The progress bar st.
            min_timestep_boundary: The min timestep boundary.
            max_timestep_boundary: The max timestep boundary.
        """
        # Parameter check
        height, width = self.check_resize_height_width(height, width)
        if num_frames % 4 != 1:
        ...
        if input_video is not None:
            ...
            input_video = self.preprocess_images(input_video)
            input_video = torch.stack(input_video, dim=2).to(dtype=self.torch_dtype, device=self.device)
            clean_latents = self.encode_video(input_video, **tiler_kwargs).to(dtype=self.torch_dtype, device=self.device)
            timestep_id = torch.randint(int(min_timestep_boundary * self.scheduler.num_train_timesteps), int(max_timestep_boundary * self.scheduler.num_train_timesteps), (1,)) 
            timestep = self.scheduler.timesteps[timestep_id].to(dtype=self.torch_dtype, device=self.device)
            latents = self.scheduler.add_noise(clean_latents, noise, timestep)
        else:
            latents = noise
        ...
        if input_image is None or not self.dit.require_vae_embedding:
            ...
            # print("encoding image...")
            self.load_models_to_device(["image_encoder", "vae"])
            image_emb = self.encode_image(input_image, end_image, num_frames, height, width, mid_images_list=mid_images_list, mid_images_id_list=mid_images_id_list, **tiler_kwargs)
        ...
        noise_pred_posi = model_fn_wan_video(
            ...
            x=latents, timestep=timestep,
            **prompt_emb_posi, **image_emb, **extra_input,
            **tea_cache_posi, **usp_kwargs, **motion_kwargs, **vace_kwargs, **reference_image_kwargs, **extra_kwargs
            
        )
        assert noise_pred_posi is None # because extra_kwargs is not None
        return clean_latents, prompt_emb_posi, image_emb
    ...
    def cache(
        ...
        if input_video is not None:
            ...
            raise ValueError('input_video is None')
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    # Image Embedding
    # print("y is not None? {}. dit.require_vae_embedding = {}".format(y is not None, dit.require_vae_embedding))
    ...
    x = dit.patchify(x, control_camera_latents_input) # , (f, h, w)
    f, h, w = x.shape[2:]
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            if "file_name" in kwargs.keys() and block_id == 15:
                tensor_to_save = x.detach().cpu().contiguous()
                if not os.path.exists(kwargs['save_path']):
                    os.makedirs(kwargs['save_path'])
                save_file({"hidden_states": tensor_to_save}, os.path.join(kwargs['save_path'], "{}---{}.safetensors".format(kwargs['file_name'], timestep.item())))
                print(os.path.join(kwargs['save_path'], "{}---{}.safetensors".format(kwargs['file_name'], timestep.item())), "Saved")
                return None
