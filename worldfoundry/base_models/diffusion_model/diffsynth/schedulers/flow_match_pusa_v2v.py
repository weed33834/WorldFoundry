"""Module for base_models -> diffusion_model -> diffsynth -> schedulers -> flow_match_pusa_v2v.py functionality."""

import torch

class FlowMatchSchedulerPusaV2V():
    """Flow match scheduler pusa v implementation."""
    ...
    def step(self, model_output, timestep, sample, to_final=False, cond_frame_latent_indices=None, noise_multipliers=None, **kwargs):
        ...
        Args:
            ...
            cond_frame_latent_indices: The cond frame latent indices.
            noise_multipliers: The noise multipliers.
        """
    ...
    def add_noise_for_conditioning_frames(self, original_samples, noise, timestep, noise_multiplier=None):
        """Add noise for conditioning frames.

        Args:
            original_samples: The original samples.
            noise: The noise.
            timestep: The timestep.
            noise_multiplier: The noise multiplier.
        """
        if isinstance(timestep, torch.Tensor):
            self.timesteps = self.timesteps.to(timestep.device)
            self.sigmas = self.sigmas.to(timestep.device)
