...
def rescale_depthmap(
    depthmap, output_resolution, force=True
):
    """Rescale depthmap.
    ...
    # define output resolution
    assert output_resolution.shape == (2,)
    input_resolution = np.array(depthmap.shape[::-1])
    scale_final = np.max(output_resolution / input_resolution) + 1e-8
    if scale_final >= 1 and not force:  # image is already smaller than what is asked
        return depthmap
    output_resolution = np.floor(input_resolution * scale_final).astype(int)
    depthmap = cv2.resize(
...
def rescale_flowmap(
    flowmap, output_resolution, force=True
):
    """Rescale flowmap.
    ...
    # define output resolution
    assert output_resolution.shape == (2,)
    input_resolution = np.array(flowmap.shape[1::-1])
    scale_final = np.max(output_resolution / input_resolution) + 1e-8
    if scale_final >= 1 and not force:  # image is already smaller than what is asked
        return flowmap
    output_resolution = np.floor(input_resolution * scale_final).astype(int)
    flowmap = cv2.resize(
        flowmap.astype("float32"),
        output_resolution,
        interpolation=cv2.INTER_LINEAR,
    )
    origin_w, origin_h = input_resolution
    new_w, new_h = output_resolution
    flowmap[..., 0] *= new_w / origin_w
    flowmap[..., 1] *= new_h / origin_h
    return flowmap


def rescale_3dflowmap(
    flowmap, output_resolution, force=True
):
    """Rescale 3dflowmap.
