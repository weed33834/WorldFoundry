"""Module for base_models -> diffusion_model -> diffsynth -> auxiliary_models -> worldmirror -> models -> models -> visual_transformer.py functionality."""
...
from ..layers.block import Block, CrossBlock
...
class VisualGeometryTransformer(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            enable_cond: The enable cond.
            enable_motion: The enable motion.
            sampling_strategy: The sampling strategy.
            fixed_patch_embed: The fixed patch embed.
            condition_strategy: The condition strategy.
            intermediate_idxs: The intermediate idxs.
        """
        super().__init__()
        ...
        # Initialize bidirectional motion-encoding branch if enabled
        if self.enable_motion:
            self._init_motion_branch(block_fn, embed_dim, num_heads, mlp_ratio, qkv_bias, proj_bias, ffn_bias, init_values, qk_norm)
    ...
    def _init_rotary_position_embedding(self, rope_freq):
        """Helper function to init rotary position embedding.
    ...
    def _init_transformer_blocks(self, block_fn, embed_dim, num_heads, mlp_ratio, qkv_bias, proj_bias, ffn_bias, init_values, qk_norm):
        """Helper function to init transformer blocks.
    ...
    def _init_motion_branch(self, block_fn, embed_dim, num_heads, mlp_ratio, qkv_bias, proj_bias, ffn_bias, init_values, qk_norm):
        """Helper function to init motion branch.
        ...
        Args:
            ...
            qk_norm: The qk norm.
        """
        self.motion_tokens = nn.Parameter(torch.randn(1, 2, 1, embed_dim))
        nn.init.normal_(self.motion_tokens, std=1e-6)
        self.motion_embeds = nn.ModuleList(
            [
                nn.Linear(2 * embed_dim, embed_dim, bias=True)
                for _ in range(len(self.intermediate_idxs))
            ]
        )
        self.motion_self_blocks = nn.ModuleList([
            block_fn(
                dim=embed_dim,
                num_heads=num_heads,
                mlp_ratio=mlp_ratio,
        ...
        self.motion_fwd_blocks = nn.ModuleList([
            CrossBlock(
                dim=embed_dim,
                num_heads=num_heads,
                mlp_ratio=mlp_ratio,
        ...
        self.motion_bwd_blocks = nn.ModuleList([
            CrossBlock(
                dim=embed_dim,
                num_heads=num_heads,
                mlp_ratio=mlp_ratio,
        ...
        with torch.amp.autocast('cuda', dtype=torch.bfloat16):
            ...
            if self.enable_motion and use_motion:
                ...
                for idx in range(len(outputs)):
                    ...
                    fwd_output_list.append(fwd_output)
                    bwd_output_list.append(bwd_output)

        return outputs, self.patch_start_idx, fwd_output_list, bwd_output_list
    ...
    def _process_motion_branch(self, tokens1, tokens2, pos_emb1, pos_emb2, layer_idx):
        """Process motion branch with forward and backward cross-attention."""
        b, seq_len, patch_count, embed_dim = tokens1.shape
        tokens1 = tokens1.reshape(b * seq_len, patch_count, embed_dim)
        tokens2 = tokens2.reshape(b * seq_len, patch_count, embed_dim)
        combined_tokens = torch.cat([tokens1, tokens2], dim=1)
        pos_emb1 = pos_emb1.reshape(b * seq_len, patch_count, 2)
        pos_emb2 = pos_emb2.reshape(b * seq_len, patch_count, 2)
        combined_pos = torch.cat([pos_emb1, pos_emb2], dim=1)

        if self.training:
            combined_tokens = checkpoint(self.motion_self_blocks[layer_idx], combined_tokens, pos=combined_pos, use_reentrant=self.use_reentrant)
        else:
            combined_tokens = self.motion_self_blocks[layer_idx](combined_tokens, pos=combined_pos)

        tokens1, tokens2 = combined_tokens.chunk(2, dim=1)

        if self.training:
            fwd_tokens = checkpoint(
                self.motion_fwd_blocks[layer_idx], tokens1, tokens2, pos_emb1, pos_emb2, use_reentrant=self.use_reentrant
            )
            bwd_tokens = checkpoint(
                self.motion_bwd_blocks[layer_idx], tokens2, tokens1, pos_emb2, pos_emb1, use_reentrant=self.use_reentrant
            )
        else:
            fwd_tokens = self.motion_fwd_blocks[layer_idx](tokens1, tokens2, pos_emb1, pos_emb2)
            bwd_tokens = self.motion_bwd_blocks[layer_idx](tokens2, tokens1, pos_emb2, pos_emb1)
        fwd_tokens = fwd_tokens.reshape(b, seq_len, patch_count, embed_dim)
        bwd_tokens = bwd_tokens.reshape(b, seq_len, patch_count, embed_dim)
        return fwd_tokens, bwd_tokens
