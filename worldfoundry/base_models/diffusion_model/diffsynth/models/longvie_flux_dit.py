"""Module for base_models -> diffusion_model -> diffsynth -> models -> longvie_flux_dit.py functionality."""
...
class RoPEEmbedding(torch.nn.Module):
    """Ro pe embedding implementation."""
...
class FluxJointAttention(torch.nn.Module):
    """Flux joint attention implementation."""
...
class FluxJointTransformerBlock(torch.nn.Module):
    """Flux joint transformer block implementation."""
...
class FluxSingleAttention(torch.nn.Module):
    """Flux single attention implementation."""
    ...
    def forward(self, hidden_states, image_rotary_emb):
        ...
        hidden_states = _worldfoundry_scaled_dot_product_attention(q, k, v)
        hidden_states = hidden_states.transpose(1, 2).reshape(batch_size, -1, self.num_heads * self.head_dim)
        hidden_states = hidden_states.to(q.dtype)
        return hidden_states
...
class AdaLayerNormSingle(torch.nn.Module):
    """Ada layer norm single implementation."""
...
class FluxSingleTransformerBlock(torch.nn.Module):
    """Flux single transformer block implementation."""
    def __init__(self, dim, num_attention_heads):
        """Init.
...
class AdaLayerNormContinuous(torch.nn.Module):
    """Ada layer norm continuous implementation."""
    def __init__(self, dim):
        """Init.
...
class FluxDiT(torch.nn.Module):
    """Flux di t implementation."""
    def __init__(self, disable_guidance_embedder=False, input_dim=64, num_blocks=19):
        """Init.

        Args:
            disable_guidance_embedder: The disable guidance embedder.
    ...
    def construct_mask(self, entity_masks, prompt_seq_len, image_seq_len):
        ...
        Args:
            ...
            prompt_seq_len: The prompt seq len.
...
class FluxDiTStateDictConverter:
    """Flux di t state dict converter implementation."""
    def __init__(self):
        """Init."""
        pass
