"""Module for base_models -> diffusion_model -> diffsynth -> models -> wan_video_dit_dual_control.py functionality."""
...
class WanModelDualControl(torch.nn.Module):
    """Wan model dual control implementation."""
    def __init__(
        self,
        dim: int,
        ffn_dim: int,
        ...
        self.control_blocks_dense = nn.ModuleList([
            DiTBlock(has_image_input, dim//2, num_heads//2, ffn_dim//2, eps)
        ...
        self.control_initial_combine_linear_dense.weight.data.zero_()
        self.control_initial_combine_linear_dense.bias.data.zero_()

        self.control_initial_combine_linear_sparse = torch.nn.Linear(dim, dim//2)
        self.control_initial_combine_linear_sparse.weight.data.zero_()
        self.control_initial_combine_linear_sparse.bias.data.zero_()
        self.control_text_linear = torch.nn.Linear(dim, dim//2)
        self.control_t_mod = torch.nn.Linear(dim, dim//2)
        # self.control_text_linears_map = torch.nn.ModuleList([torch.nn.Linear(dim, dim//2) for _ in range(num_layers//2)])
        # self.control_t_mod_map = torch.nn.ModuleList([torch.nn.Linear(dim, dim//2) for _ in range(num_layers//2)])
        self.control_combine_linears = torch.nn.ModuleList([torch.nn.Linear(dim//2, dim) for _ in range(self.control_layers)])
        for linear in self.control_combine_linears:
