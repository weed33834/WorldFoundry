"""Module for base_models -> diffusion_model -> diffsynth -> models -> longvie_wan_video_vae.py functionality."""
...
class WanVideoVAE(nn.Module):
    ...
    @staticmethod
    def _matches_device(actual, target):
        """Helper function to matches device.
        ...
        if actual.type != target.type:
            return False
        return target.index is None or actual.index == target.index

    def _ensure_model_on_device(self, device, dtype=None):
        """Helper function to ensure model on device.

        Args:
            device: The device.
            dtype: The dtype.
        """
        if getattr(self, "vram_management_enabled", False):
        ...
        target_dtype = dtype if dtype is not None and param.is_floating_point() else None
        needs_device = not self._matches_device(param.device, device)
        needs_dtype = target_dtype is not None and param.dtype != target_dtype
    ...
    def tiled_decode(self, hidden_states, device, tile_size, tile_stride):
        ...
        self._ensure_model_on_device(computation_device, dtype=hidden_states.dtype)
    ...
    def tiled_encode(self, video, device, tile_size, tile_stride):
        ...
        self._ensure_model_on_device(computation_device, dtype=video.dtype)
    ...
    def single_encode(self, video, device):
        ...
        self._ensure_model_on_device(video.device, dtype=video.dtype)
    ...
    def single_decode(self, hidden_state, device):
        ...
        self._ensure_model_on_device(hidden_state.device, dtype=hidden_state.dtype)
