"""Module for base_models -> diffusion_model -> diffsynth -> models -> step1x_connector.py functionality."""
...
def attention(q, k, v, attn_mask, mode="torch"):
    ...
    k = k.transpose(1, 2)
    v = v.transpose(1, 2)
    x = _worldfoundry_scaled_dot_product_attention(q, k, v, attn_mask=attn_mask)
...
class CrossAttnBlock(torch.nn.Module):
    """Cross attn block implementation."""
    def __init__(
        self,
        hidden_size,
        heads_num,
...
class Qwen2ConnectorStateDictConverter:
    """Qwen connector state dict converter implementation."""
    def __init__(self):
        """Init."""
        pass
