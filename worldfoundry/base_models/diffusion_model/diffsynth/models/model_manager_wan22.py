"""Module for base_models -> diffusion_model -> diffsynth -> models -> model_manager_wan22.py functionality."""
...
from worldfoundry.core.model_loading import load_state_dict, hash_state_dict_keys, split_state_dict_with_prefix
...
class ModelDetectorFromSingleFile:
    ...
    def _select_metadata(self, candidates, allowed_model_names=None):
        if not candidates:
            return None
        if not allowed_model_names:
            return candidates[0]
        allowed = set(allowed_model_names)
        for model_names, model_classes, model_resource in candidates:
            if any(name in allowed for name in model_names):
    ...
    def load(self, file_path="", state_dict={}, device="cuda", torch_dtype=torch.float16, **kwargs):
        ...
        keys_hash_with_shape = hash_state_dict_keys(state_dict, with_shape=True)
        if keys_hash_with_shape in self.keys_hash_with_shape_dict:
            metadata = self._select_metadata(self.keys_hash_with_shape_dict[keys_hash_with_shape], allowed_model_names)
        ...
        keys_hash = hash_state_dict_keys(state_dict, with_shape=False)
        if keys_hash in self.keys_hash_dict:
            metadata = self._select_metadata(self.keys_hash_dict[keys_hash], allowed_model_names)
            if metadata is not None:
                model_names, model_classes, model_resource = metadata
                loaded_model_names, loaded_models = load_model_from_single_file(state_dict, model_names, model_classes, model_resource, torch_dtype, device)
                return loaded_model_names, loaded_models
...
class ModelManagerWan22:
    """Model manager wan implementation."""
    def __init__(
        self,
