...
"""Module for base_models -> diffusion_model -> image -> sana -> diffusion -> data -> wids -> wids.py functionality."""
...
def update_dict_with_extend(original_dict, update_dict):
    """Update dict with extend.

    Args:
        original_dict: The original dict.
        update_dict: The update dict.
    """
...
def hash_localname(dldir=None):
    """Hash localname.

    Args:
        dldir: The dldir.
    """
    dldir = dldir or os.path.join(tempfile.gettempdir(), "_wids_cache")
...
class DistributedLocalSampler(DistributedSampler):
    """Distributed local sampler implementation."""
    def __iter__(self):
        """Iter."""
        if self.shuffle:
