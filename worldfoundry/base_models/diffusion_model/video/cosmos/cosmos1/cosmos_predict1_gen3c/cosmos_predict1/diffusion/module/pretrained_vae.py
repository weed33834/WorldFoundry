...
"""Module for base_models -> diffusion_model -> video -> cosmos -> cosmos1 -> cosmos_predict1_gen3c -> cosmos_predict1 -> diffusion -> module -> pretrained_vae.py functionality."""
...
class BasePretrainedImageVAE(BaseVAE):
    ...
    def __init__(
        ...
        Args:
            name: The name.
            latent_ch: The latent ch.
            is_image: The is image.
            is_bf16: The is bf16.
    ...
    def register_mean_std(self, vae_dir: str) -> None:
        ...
        Returns:
            The return value.
        """
        stats_path = os.path.join(vae_dir, "image_mean_std.pt")
        if not os.path.isfile(stats_path):
            stats_path = os.path.join(vae_dir, "mean_std.pt")
        latent_mean, latent_std = torch.load(stats_path, weights_only=True)
        if latent_mean.numel() != self.latent_ch:
            latent_mean = latent_mean.view(self.latent_ch, -1)[:, 0]
            latent_std = latent_std.view(self.latent_ch, -1)[:, 0]
...
class BasePretrainedVideoTokenizer(ABC):
    ...
    def __init__(
        ...
        Args:
            pixel_chunk_duration: The pixel chunk duration.
            temporal_compress_factor: The temporal compress factor.
            max_enc_batch_size: The max enc batch size.
            max_dec_batch_size: The max dec batch size.
        """
...
class VideoJITTokenizer(BasePretrainedVideoTokenizer, JITVAE, VideoTokenizerInterface):
    ...
    def __init__(
        ...
        Args:
            ...
            temporal_compression_factor: The temporal compression factor.
            pixel_chunk_duration: The pixel chunk duration.
            max_enc_batch_size: The max enc batch size.
            max_dec_batch_size: The max dec batch size.
            spatial_resolution: The spatial resolution.
        """
        super().__init__(
...
class JointImageVideoTokenizer(BaseVAE, VideoTokenizerInterface):
    ...
    def __init__(
        ...
        Args:
            image_vae: The image vae.
            video_vae: The video vae.
            name: The name.
            latent_ch: The latent ch.
            squeeze_for_image: The squeeze for image.
        """
        super().__init__(latent_ch, name)
