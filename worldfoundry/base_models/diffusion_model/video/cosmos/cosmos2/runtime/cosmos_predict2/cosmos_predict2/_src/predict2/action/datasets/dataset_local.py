...
class Dataset_3D(Dataset):
    ...
    def _init_sequences(self, ann_files):
        """Helper function to init sequences.

        Args:
            ann_files: The ann files.
        """
        samples = []
    ...
    def _filter_rollout(self):
        """Helper function to filter rollout."""
    ...
    def _load_and_process_ann_file(self, ann_file):
        """Helper function to load and process ann file.

        Args:
            ann_file: The ann file.
        """
        samples = []
        with open(ann_file, "r") as f:
            ann = json.load(f)
    ...
    def _get_robot_states(self, label, frame_ids):
        """Helper function to get robot states.
    ...
    def _get_actions(self, arm_states, gripper_states, accumulate_action):
        """Helper function to get actions.

        Args:
            arm_states: The arm states.
            gripper_states: The gripper states.
            accumulate_action: The accumulate action.
        """
...
if INTERNAL:
    ...
    if __name__ == "__main__":
        ...
        base_path_pi_benchmark_local = os.environ.get(
            "WORLDFOUNDRY_COSMOS_ACTION_DATASET_ROOT",
            "cache/worldfoundry/data/cosmos/action_dataset/single_chunk",
        )
