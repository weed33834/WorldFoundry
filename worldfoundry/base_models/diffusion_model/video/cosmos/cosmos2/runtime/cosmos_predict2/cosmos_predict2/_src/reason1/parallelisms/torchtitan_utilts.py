...
"""Module for base_models -> diffusion_model -> video -> cosmos -> cosmos2 -> runtime -> cosmos_predict2 -> cosmos_predict2 -> _src -> reason1 -> parallelisms -> torchtitan_utilts.py functionality."""
...
def _warn_overwrite_env(env, val):
    """Helper function to warn overwrite env.
...
def create_context_parallel_ctx(
    ...
    """Create context parallel ctx.

    Args:
        cp_mesh: The cp mesh.
        cp_buffers: The cp buffers.
        cp_seq_dims: The cp seq dims.
        cp_no_restore_buffers: The cp no restore buffers.
        cp_rotate_method: The cp rotate method.
    """
    try:
...
def get_train_context(enable_loss_parallel: bool, enable_compiled_autograd: bool):
    ...
    Args:
        enable_loss_parallel: The enable loss parallel.
        enable_compiled_autograd: The enable compiled autograd.
    """
    ...
    def context(cp_context: Optional[Generator[None, None, None]] = None):
        ...
        with contextlib.ExitStack() as stack:
            ...
            if cp_context is not None:
                ...
                stack.enter_context(attention_backend_context(backends=["flash", "efficient"]))
...
def _get_distributed_backend(job_config):
    """Helper function to get distributed backend.
...
def get_num_params(model: torch.nn.Module, exclude_embedding: bool = False) -> int:
    ...
    Args:
        model: The model.
        exclude_embedding: The exclude embedding.
...
def get_num_flop_per_token(num_params: int, model_config, seq_len) -> int:
    """Get num flop per token.

    Args:
        num_params: The num params.
