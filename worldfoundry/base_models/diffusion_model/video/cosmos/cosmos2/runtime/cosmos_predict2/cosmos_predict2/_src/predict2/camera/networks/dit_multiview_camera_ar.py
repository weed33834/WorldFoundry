...
"""Module for base_models -> diffusion_model -> video -> cosmos -> cosmos2 -> runtime -> cosmos_predict2 -> cosmos_predict2 -> _src -> predict2 -> camera -> networks -> dit_multiview_camera_ar.py functionality."""
...
class CameraARInferenceDiT(InferenceCheckpointModule):
    """
...
class CameraARInferenceDiTWithConditionalMask(CameraARInferenceDiT):
    """Camera ar mini train di twith conditional mask implementation."""
...
def replace_selfattn_op_with_sparse_attn_op(
    model: CameraARInferenceDiT, n_dense_blocks: int = 0, natten_parameters: Union[dict, list] = None
) -> CameraARInferenceDiT:
    """
    Replace the self-attention operator with a sparse self-attention operator.

    Args:
        model: CameraARInferenceDiT instance
