...
"""Module for base_models -> diffusion_model -> video -> cosmos -> cosmos2 -> runtime -> cosmos_predict2 -> cosmos_predict2 -> _src -> predict2 -> tokenizers -> wan2pt1.py functionality."""
...
class WanVAE_(nn.Module):
    ...
    def encode(self, x, scale, clear_encoder_cache=True):
        ...
        Args:
            x: The x.
            scale: The scale.
            clear_encoder_cache: The clear encoder cache.
        """
    ...
    def decode(self, z, scale, clear_decoder_cache=True):
        ...
        Args:
            z: The z.
            scale: The scale.
            clear_decoder_cache: The clear decoder cache.
        """
...
def _video_vae(
    ...
    **kwargs,
    ...
    if VALIDATION or pretrained_path is None:
...
class WanVAE:
    ...
    def __init__(
        ...
        Args:
            ...
            load_mean_std: The load mean std.
            mean_std_path: The mean std path.
            dtype: The dtype.
            device: The device.
            is_amp: The is amp.
            benchmark: The benchmark.
            temporal_window: The temporal window.
            is_parallel: The is parallel.
            cp_grid_shape: The cp grid shape.
        """
        self.dtype = dtype
        self.device = device
    ...
    @property
    def _cp_dim(self):
        """Helper function to cp dim."""
        return 3

    def _broadcast_split_for_model_parallelsim(self, state: torch.Tensor) -> torch.Tensor:
        """Helper function to broadcast split for model parallelsim.
    ...
    def _cat_outputs_cp(self, local_video_recon: torch.Tensor):
        """Helper function to cat outputs cp.

        Args:
            local_video_recon: The local video recon.
        """
    ...
    def _disable_context_parallel(self):
        """Helper function to disable context parallel."""
        self.context_parallel_enabled = False
        for _, plugin_list in self.plugins.items():
            for _, plugin in plugin_list.items():
    ...
    def _is_image_batch(self, x: torch.Tensor) -> bool:
        """Helper function to is image batch.
