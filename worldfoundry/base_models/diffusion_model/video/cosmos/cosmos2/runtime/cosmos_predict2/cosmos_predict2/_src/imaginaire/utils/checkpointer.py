...
"""Module for base_models -> diffusion_model -> video -> cosmos -> cosmos2 -> runtime -> cosmos_predict2 -> cosmos_predict2 -> _src -> imaginaire -> utils -> checkpointer.py functionality."""
...
from cosmos_predict2._src.imaginaire.utils import callback, log, misc, object_store
...
class MultiRankCheckpointer(Checkpointer):
    """Multi rank checkpointer implementation."""
    def save(
        self,
        model: ImaginaireModel,
        optimizer: torch.optim.Optimizer,
...
def non_strict_load_model(model: torch.nn.Module, checkpoint_state_dict: dict) -> _IncompatibleKeys:
    """Non strict load model.

    Args:
        model: The model.
        checkpoint_state_dict: The checkpoint state dict.
