"""Module for base_models -> diffusion_model -> video -> skyreels_v3 -> generate_video.py functionality."""
...
from worldfoundry.core.io import download_to_cache
...
def maybe_download(path_or_url: str, save_dir: str) -> str:
    ...
    cached = download_to_cache(path_or_url, cache_dir=Path(save_dir))
    logging.info(f"resolved input: {cached}")
    return str(cached)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output directory or .mp4 path for the generated video.",
    )
    ...
    if args.task_type == "single_shot_extension":
        ...
        from skyreels_v3.pipelines.shot_switching_extension_pipeline import ShotSwitchingExtensionPipeline
        ...
        from skyreels_v3.pipelines.talking_avatar_pipeline import TalkingAvatarPipeline
    ...
    if local_rank == 0:
        ...
        if args.output_dir:
            ...
            if output_target.suffix.lower() == ".mp4":
                ...
                save_dir = str(output_target.parent)
            else:
                save_dir = str(output_target)
