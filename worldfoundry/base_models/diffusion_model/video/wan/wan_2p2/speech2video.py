...
"""Module for base_models -> diffusion_model -> video -> wan -> wan_2p2 -> speech2video.py functionality."""
...
from worldfoundry.core.attention.rope_sequence_parallel import sp_attn_forward, sp_dit_forward
...
class WanS2V:
    ...
    def prepare_default_cond_input(self,
                                   ...
                                   enable_kp=False,
                                   enable_pose=False):
        """Prepare default cond input.

        Args:
            map_shape: The map shape.
            motion_frames: The motion frames.
            lat_motion_frames: The lat motion frames.
            enable_mano: The enable mano.
            enable_kp: The enable kp.
            enable_pose: The enable pose.
        """
    ...
    def load_pose_cond(self, pose_video, num_repeat, infer_frames, size):
        ...
        Args:
            pose_video: The pose video.
            num_repeat: The num repeat.
            infer_frames: The infer frames.
            size: The size.
        """
        HEIGHT, WIDTH = size
    ...
    def get_gen_size(self, size, max_area, ref_image_path, pre_video_path):
        ...
        Args:
            size: The size.
            max_area: The max area.
            ref_image_path: The ref image path.
            pre_video_path: The pre video path.
        """
        if not size is None:
            HEIGHT, WIDTH = size
        else:
    ...
    def tts(self, tts_prompt_audio, tts_prompt_text, tts_text):
        ...
        Args:
            tts_prompt_audio: The tts prompt audio.
            tts_prompt_text: The tts prompt text.
            tts_text: The tts text.
        """
    ...
    def load_tts(self):
        ...
        if not os.path.exists('CosyVoice'):
            from .utils.utils import download_cosyvoice_repo
            download_cosyvoice_repo('CosyVoice')
        if not os.path.exists('CosyVoice2-0.5B'):
            from .utils.utils import download_cosyvoice_model
