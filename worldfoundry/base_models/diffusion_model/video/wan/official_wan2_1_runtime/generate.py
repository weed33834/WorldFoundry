...
"""Module for base_models -> diffusion_model -> video -> wan -> official_wan2_1_runtime -> generate.py functionality."""
...
from worldfoundry.base_models.diffusion_model.video.wan.shared.media_utils import cache_image, cache_video, str2bool
...
def generate(args):
    ...
    if "t2v" in args.task or "t2i" in args.task:
        ...
        if args.image is None:
            raise ValueError("--image is required for Wan image-to-video inference.")
        logging.info(f"Input prompt: {args.prompt}")
        ...
        if args.first_frame is None or args.last_frame is None:
            raise ValueError("--first_frame and --last_frame are required for Wan FLF2V inference.")
        logging.info(f"Input prompt: {args.prompt}")
