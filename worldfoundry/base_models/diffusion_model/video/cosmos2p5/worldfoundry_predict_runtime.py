"""Module for base_models -> diffusion_model -> video -> cosmos2p5 -> worldfoundry_predict_runtime.py functionality."""
...
from worldfoundry.base_models.diffusion_model.video.cosmos2p5.artifacts import (
    find_local_artifact_path,
    resolve_local_artifact_path,
)
from worldfoundry.base_models.diffusion_model.video.cosmos2p5.modules import (
    Cosmos25Transformer3DModel,
    Reason1TextEncoder,
)
from worldfoundry.base_models.diffusion_model.video.cosmos2p5.utils.registry import COSMOS_2P5_TASKS
from worldfoundry.base_models.diffusion_model.video.cosmos2p5.utils.utils import (
    get_cosmos_config_for_path,
...
_TRANSFORMER_CHECKPOINT_RELATIVE_PATHS = (
    ...
    "base/post-trained/e21d2a49-2968-4690-8a1d-10d75622fa0c_ema_bf16.pt",
    "base/post-trained/54937b8c-0e1f-4a0d-8f76-6c6902a04b5f_ema_bf16.pt",
    "base/post_trained/81edfebe-bd6a-4039-8c1d-737df1a790bf_ema_bf16.pt",
    "base/post_trained/e21d2a49-2968-4690-8a1d-10d75622fa0c_ema_bf16.pt",
    "base/post_trained/54937b8c-0e1f-4a0d-8f76-6c6902a04b5f_ema_bf16.pt",
    "base/post-trained/*_ema_bf16.pt",
    "base/post_trained/*_ema_bf16.pt",
)


class CosmosPredict2p5Runtime:
    """Cosmos predict p runtime implementation."""
    ...
    def plan(
        cls,
        transformer_model_path: str = "nvidia/Cosmos-Predict2.5-2B",
        text_encoder_model_path: str = "nvidia/Cosmos-Reason1-7B",
        vae_model_path: str = "Wan-AI/Wan2.1-T2V-1.3B",
    ) -> dict[str, Any]:
        """
        Inspect local Cosmos Predict2.5 artifacts without loading weights.

        Args:
            transformer_model_path: Hugging Face repo id or local transformer checkpoint path.
            text_encoder_model_path: Hugging Face repo id or local text encoder directory.
            vae_model_path: Hugging Face repo id or local VAE checkpoint path.
        """
        transformer_ckpt = find_local_artifact_path(
            transformer_model_path,
            _TRANSFORMER_CHECKPOINT_RELATIVE_PATHS,
        )
        text_encoder_path = find_local_artifact_path(text_encoder_model_path)
        vae_ckpt = find_local_artifact_path(vae_model_path, ("Wan2.1_VAE.pth",))
        blockers = []
        if transformer_ckpt is None:
            blockers.append("Missing Cosmos Predict2.5 transformer checkpoint.")
        if text_encoder_path is None:
            blockers.append("Missing Cosmos Reason1 text encoder.")
        if vae_ckpt is None:
            blockers.append("Missing Wan2.1 VAE checkpoint.")
        return {
            "runtime": "in-tree",
            "mode": "img2world",
    ...
    def from_pretrained(
        ...
        transformer_ckpt = resolve_local_artifact_path(
            transformer_model_path,
            _TRANSFORMER_CHECKPOINT_RELATIVE_PATHS,
        )
        text_encoder_path = resolve_local_artifact_path(text_encoder_model_path)
