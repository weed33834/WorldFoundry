...
from diffusers.utils import BaseOutput, is_cosmos_guardrail_available, logging
...
class Cosmos3OmniPipeline(DiffusionPipeline):
    ...
    def _encode_video(self, x: torch.Tensor) -> torch.Tensor:
        ...
        raw_mu = retrieve_latents(self.vae.encode(x.to(device=vae_device, dtype=dtype)), sample_mode="argmax")
    ...
    def decode_sound(self, latent: torch.Tensor) -> torch.Tensor:
        ...
        """
        decoder_parameter = next(self.sound_tokenizer.parameters())
        waveform = self.sound_tokenizer.decode(
            latent.unsqueeze(0).to(device=decoder_parameter.device, dtype=decoder_parameter.dtype)
        )  # [1, audio_ch, N]
    ...
    def __call__(
        ...
        if output_type == "latent":
            ...
            dtype = self.vae.dtype
            vae_device = next(self.vae.parameters()).device
            mean = self._vae_latents_mean.to(device=vae_device, dtype=dtype)
            inv_std = self._vae_latents_inv_std.to(device=vae_device, dtype=dtype)
            z_raw = latents.to(device=vae_device, dtype=dtype) / inv_std.view(1, -1, 1, 1, 1) + mean.view(
                1, -1, 1, 1, 1
            )
