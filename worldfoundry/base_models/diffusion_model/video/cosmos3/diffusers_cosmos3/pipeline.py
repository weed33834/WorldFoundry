...
"""Module for base_models -> diffusion_model -> video -> cosmos3 -> diffusers_cosmos3 -> pipeline.py functionality."""
...
from worldfoundry.base_models.diffusion_model.video.cosmos3.diffusers_cosmos3.transformer import (
...
def _sample_args_root() -> Path:
    """Helper function to sample args root.

    Returns:
        The return value.
    """
    return resolve_data_path("models", "runtime", "configs", "cosmos3", "sample_args")
...
def save_img_or_video(sample, save_fp_wo_ext, fps=24, quality=10, ffmpeg_params=None, **kwargs):
    ...
    Args:
        sample: The sample.
        save_fp_wo_ext: The save fp wo ext.
        fps: The fps.
        quality: The quality.
        ffmpeg_params: The ffmpeg params.
    """
...
class Cosmos3OmniDiffusersPipeline(DiffusionPipeline):
    """Cosmos omni diffusers pipeline implementation."""
    ...
    def __init__(
        ...
        Args:
            ...
            vision_encoder: The vision encoder.
        """
        super().__init__()
    ...
    def apply_timestep_embeds_to_noisy_tokens(
        ...
        """Apply timestep embeds to noisy tokens.

        Args:
            packed_tokens: The packed tokens.
            packed_timestep_embeds: The packed timestep embeds.
            noisy_frame_indexes: The noisy frame indexes.
            token_shapes: The token shapes.
    ...
    def patchify_and_pack_latents(
        ...
        Args:
            ...
            tokens_vision: The tokens vision.
            token_shapes_vision: The token shapes vision.
    ...
    def unpatchify_and_unpack_latents(
        ...
        """Unpatchify and unpack latents.

        Args:
            latent_patch_size: The latent patch size.
            latent_channel: The latent channel.
            packed_mse_preds: The packed mse preds.
            token_shapes_vision: The token shapes vision.
            noisy_frame_indexes_vision: The noisy frame indexes vision.
            original_latent_shapes: The original latent shapes.
    ...
    def normalize_video_databatch_inplace(
        ...
        """Normalize video databatch inplace.

        Args:
            input_video_key: The input video key.
    ...
    def augment_image_dim_inplace(
        ...
        """Augment image dim inplace.

        Args:
            input_image_key: The input image key.
            data_batch: The data batch.
            input_key: The input key.
            device: The device.
            dtype: The dtype.
    ...
    def remove_padding_from_latent(
        ...
        Args:
            spatial_compression_factor: The spatial compression factor.
            x0_tokens_vision: The x0 tokens vision.
    ...
    def get_inference_text_tokens(
        ...
        """Get inference text tokens.

        Args:
            use_system_prompt: The use system prompt.
            input_caption_key: The input caption key.
            data_batch: The data batch.
            has_negative_prompt: The has negative prompt.
    ...
    def derive_include_end_of_generation_token(self, joint_attn_implementation: str) -> bool:
        """Derive include end of generation token.
    ...
    def prepare_inference_data(
        ...
        Args:
            ...
            width: The width.
            fps: The fps.
            condition_frame_indexes: The condition frame indexes.
            noises: The noises.
    ...
    def get_cfg_velocity(
        ...
        Args:
            ...
            gen_data_clean: The gen data clean.
            sequence_plans: The sequence plans.
            cond_tokens: The cond tokens.
            uncond_tokens: The uncond tokens.
            include_eog: The include eog.
            ...
            use_moe: The use moe.
            joint_attn_implementation: The joint attn implementation.
            skip_text_tokens_for_cfg: The skip text tokens for cfg.
            normalize_cfg: The normalize cfg.
            device: The device.
            dtype: The dtype.
        ...
        def _run_cond(text_tokens: list[list[int]], skip_text: bool) -> torch.Tensor:
            """Helper function to run cond.

            Args:
                text_tokens: The text tokens.
                skip_text: The skip text.
    ...
    def _resolve_defaults_and_prompts(
        ...
        sample_args_root = _sample_args_root()
        defaults = json.loads((sample_args_root / f"{model_mode}.json").read_text())
        ...
        if negative_prompt_base is None and defaults.get("negative_prompt_file"):
            neg_path = sample_args_root / str(defaults["negative_prompt_file"])
            negative_prompt_base = json.dumps(json.loads(neg_path.read_text()))
        if negative_prompt_base is None:
    ...
    def decode_latents(self, vision_list: list[torch.Tensor]) -> list[torch.Tensor]:
        ...
        for vision_latent in vision_list:
            device = getattr(self, "_execution_device", vision_latent.device)
            vision = self.vision_tokenizer.decode(vision_latent.to(device))  # [1, C, T, H, W]
