...
from worldfoundry.base_models.diffusion_model.video.lvdm.models.ddpm3d import (
    LatentDiffusion,
)
...
class T2VTurboVC2Pipeline(DiffusionPipeline):
    def __init__(
        ...
        self.set_runtime_dtype(next(pretrained_t2v.parameters()).dtype)

    @property
    def dtype(self) -> torch.dtype:
        return getattr(self, "_worldfoundry_dtype", next(self.unet.parameters()).dtype)

    def set_runtime_dtype(self, dtype: torch.dtype) -> None:
        self._worldfoundry_dtype = dtype
        for module in (self.pretrained_t2v, self.vae, self.unet, self.text_encoder):
