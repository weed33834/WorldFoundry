...
# HunyuanWorld-Voyager keeps using the in-repo MoGe-1 implementation.
...
def _checkpoint_root() -> str:
    return os.path.expanduser(os.environ.get("WORLDFOUNDRY_CHECKPOINT_ROOT") or os.path.join(os.path.dirname(_project_root()), "ckpt"))


def _resolve_moge_v1_root(pretrained_model_path: str | os.PathLike | None) -> str:
    if pretrained_model_path:
        candidate = os.path.expanduser(os.path.expandvars(str(pretrained_model_path)))
    ...
    names = [
        str(pretrained_model_path or DEFAULT_HUNYUAN_WORLD_VOYAGER_MOGE1_REPO).replace("/", "--"),
        str(pretrained_model_path or DEFAULT_HUNYUAN_WORLD_VOYAGER_MOGE1_REPO).split("/")[-1],
        "Ruicheng--moge-vitl",
        "moge-vitl",
    ]
    seen = set()
    for name in names:
        if not name or name in seen:
            continue
        seen.add(name)
        for base in (ckpt_root, os.path.join(ckpt_root, "hfd")):
            candidate = os.path.join(base, name)
            if os.path.isfile(os.path.join(candidate, "model.pt")):
                return candidate
    return str(pretrained_model_path or DEFAULT_HUNYUAN_WORLD_VOYAGER_MOGE1_REPO)
...
class HunyuanWorldVoyagerRepresentation(BaseRepresentation):
    ...
    def from_pretrained(cls,
        ...
        if depth_model_name == 'moge_v1':
            model_root = _resolve_moge_v1_root(pretrained_model_path)
            if not os.path.isfile(os.path.join(model_root, "model.pt")):
