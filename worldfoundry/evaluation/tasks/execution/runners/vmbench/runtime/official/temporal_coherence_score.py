...
from worldfoundry.base_models.perception_core.segment.sam2.build_sam import build_sam2_video_predictor, build_sam2
...
from grounded_sam2_utils.track_utils import sample_points_from_masks
from grounded_sam2_utils.video_utils import create_video_from_images
from grounded_sam2_utils.common_utils import CommonUtils
from grounded_sam2_utils.mask_dictionary_model import MaskDictionaryModel, ObjectInfo

from worldfoundry.base_models.perception_core.tracking.cotracker import CoTrackerPredictor, Visualizer
from worldfoundry.base_models.perception_core.tracking.cotracker.paths import checkpoint_path as cotracker_checkpoint_path
from worldfoundry.base_models.perception_core.tracking.cotracker.visualizer import read_video_from_path
...
if __name__ == "__main__":
    ...
    """
    config_file = os.environ.get("WORLDFOUNDRY_VMBENCH_GROUNDING_DINO_CONFIG", str(grounding_dino_config_path()))
    grounded_checkpoint = os.environ.get("WORLDFOUNDRY_VMBENCH_GROUNDING_DINO_CKPT", str(grounding_dino_checkpoint_path()))
    bert_base_uncased_path = os.environ.get("WORLDFOUNDRY_GROUNDING_DINO_BERT_BASE_UNCASED", ".cache/google-bert/bert-base-uncased")
    torch.autocast(device_type="cuda", dtype=torch.bfloat16).__enter__()
    device = args.device if torch.cuda.is_available() else "cpu"
    print("device", device)

    # init sam image predictor and video predictor model
    sam2_checkpoint = os.environ.get("WORLDFOUNDRY_SAM2_CKPT", str(sam2_checkpoint_path()))
    model_cfg = os.environ.get("WORLDFOUNDRY_SAM2_CONFIG", sam2_config_name())
    ...
    cotracker_checkpoint = os.environ.get("WORLDFOUNDRY_COTRACKER2_CKPT", str(cotracker_checkpoint_path()))
