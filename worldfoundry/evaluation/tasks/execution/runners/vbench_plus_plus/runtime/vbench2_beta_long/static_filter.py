...
from worldfoundry.base_models.perception_core.optical_flow.raft import InputPadder, RAFT, checkpoint_path as raft_checkpoint_path
...
def _vbench_full_info_path():
    env_path = os.environ.get("WORLDFOUNDRY_VBENCH_FULL_INFO")
    if env_path:
        return env_path
    bundled = bundled_benchmark_asset("vbench", "VBench_full_info.json")
    if bundled.is_file():
        return str(bundled)
    return f"{CUR_PARENT_DIR}/vbench/VBench_full_info.json"
...
def static_filter(args):
    ...
    if args.filter_scope=='temporal_flickering':
        full_prompt_list = load_json(_vbench_full_info_path())
...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default=str(raft_checkpoint_path()), help="restore checkpoint")
