...
sys.modules.setdefault("vbench2_beta_trustworthiness", sys.modules[__name__])

from .utils import init_submodules, save_json, load_json
from worldfoundry.evaluation.tasks.execution.runners.vbench.runtime.vbench import VBench
import importlib
...
class VBenchTrustworthiness(VBench):
    ...
    def evaluate(self, videos_path, name, dimension_list=None, local=False, read_frame=False, custom_prompt=False):
        ...
        for dimension in dimension_list:
            try:
                dimension_module = importlib.import_module(
                    f'worldfoundry.evaluation.tasks.execution.runners.vbench_plus_plus.runtime.vbench2_beta_trustworthiness.{dimension}'
                )
