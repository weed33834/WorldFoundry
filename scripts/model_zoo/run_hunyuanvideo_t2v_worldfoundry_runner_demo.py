...
DEFAULT_OUTPUT_NAME = "hunyuanvideo_t2v_worldfoundry_cat_grass_1280x720_129f.mp4"


def default_ckpt_dir() -> Path:
    return Path(os.environ.get("WORLDFOUNDRY_CKPT_DIR", str(Path.home() / ".cache" / "worldfoundry" / "checkpoints"))) / "HunyuanVideo"
...
def default_output_dir() -> Path:
    return Path(
        os.environ.get(
            ...
            str(REPO_ROOT / "tmp" / "model_zoo" / "parity" / "hunyuanvideo"),
        )
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the WorldFoundry HunyuanVideo T2V runner demo used for model-zoo parity checks."
    )
    ...
    parser.add_argument("--model-resolution", default="720p")
...
def write_metadata(path: Path, *, args: argparse.Namespace, output_path: Path, result: dict[str, Any]) -> None:
    ...
    metadata = {
        "schema_version": "worldfoundry-hunyuanvideo-t2v-runner-demo",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "runner": "worldfoundry.synthesis.visual_generation.official_video_runtime.OfficialVideoRuntime",
        "official_reference": "Tencent-Hunyuan/HunyuanVideo sample_video.py T2V",
        "prompt": args.prompt,
        "height": args.height,
        "width": args.width,
...
def run(args: argparse.Namespace) -> int:
    if not args.ckpt_dir.is_dir():
        print(f"missing HunyuanVideo checkpoint directory: {args.ckpt_dir}", file=sys.stderr)
        return 4
